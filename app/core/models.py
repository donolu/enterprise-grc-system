...
class Subscription(models.Model):
    ...
    custom_max_users = models.PositiveIntegerField(
        null=True, blank=True, help_text="Override plan's max users limit"
    )
    custom_max_documents = models.PositiveIntegerField(
        null=True, blank=True, help_text="Override plan's max documents limit"
    )
    custom_max_frameworks = models.PositiveIntegerField(
        null=True, blank=True, help_text="Override plan's max frameworks limit"
    )
    custom_max_storage_gb = models.PositiveIntegerField(
        null=True, blank=True, help_text="Override plan's max storage limit"
    )
    enabled_modules = models.JSONField(
        default=list, blank=True, help_text="Explicit module grants for this subscription"
    )
    trial_module = models.CharField(
        max_length=40, blank=True, help_text="Single module selected for a trial subscription"
    )
    ...
    def get_effective_document_limit(self):
        ...
        return (
            self.custom_max_documents
            if self.custom_max_documents is not None
            else self.plan.max_documents
        )

    def get_effective_framework_limit(self):
        """Return custom framework limit if set, otherwise plan limit"""
        return (
            self.custom_max_frameworks
            if self.custom_max_frameworks is not None
            else self.plan.max_frameworks
        )
    ...
    def get_effective_storage_limit(self):
        ...
        return (
            self.custom_max_storage_gb if self.custom_max_storage_gb is not None else plan_storage
        )
...
class LimitOverrideRequest(models.Model):
    ...
    subscription = models.ForeignKey(
        Subscription, on_delete=models.CASCADE, related_name="limit_override_requests"
    )
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="When should temporary limits expire?"
    )
...
class Document(models.Model):
    ...
    file = models.FileField(
        upload_to=tenant_file_upload_path, help_text="Upload documents (PDF, DOC, XLS, etc.)"
    )
    ...
    is_public = models.BooleanField(
        default=False, help_text="Whether file can be accessed without authentication"
    )
