...
def test_production_settings_use_explicit_origin_allowlists():
    production_settings = PRODUCTION_SETTINGS.read_text()
    normalised_settings = production_settings.replace('"', "'")

    assert "CORS_ALLOWED_ORIGINS = _csv_env('CORS_ALLOWED_ORIGINS')" in normalised_settings
    assert (
        "CSRF_TRUSTED_ORIGINS = _csv_env('CSRF_TRUSTED_ORIGINS') or CORS_ALLOWED_ORIGINS"
        in normalised_settings
    )
...
def test_production_metrics_require_bearer_token_to_be_enabled():
    ...
    assert (
        "METRICS_BEARER_TOKEN = os.environ.get('METRICS_BEARER_TOKEN', '')" in normalised_settings
    )
...
def test_production_settings_are_not_azure_only():
    ...
    assert "_database_config_from_url(os.environ['DATABASE_URL'])" in normalised_settings
