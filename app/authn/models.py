...
class PushDevice(Device):
    ...
    device_type = models.CharField(
        max_length=20, choices=[("ios", "iOS"), ("android", "Android"), ("web", "Web Browser")]
    )
    ...
    pending_challenge = models.TextField(
        blank=True, help_text="JSON data for pending auth challenge"
    )
    ...
    def verify_token(self, token):
        ...
        try:
            ...
            if (
                challenge_data.get("challenge_id") == token
                and self.challenge_expires_at
                and self.challenge_expires_at > timezone.now()
            ):
                # Clear the challenge
