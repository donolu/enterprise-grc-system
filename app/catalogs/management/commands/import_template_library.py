...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "source_path", type=str, help="Path to a template library ZIP file or single document"
        )
        parser.add_argument(
            "--user",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Imported {imported} template documents; updated {updated}.")
        )
    ...
    def get_framework(self, framework_identifier, framework_version):
        ...
        queryset = Framework.objects.filter(name=framework_identifier) | Framework.objects.filter(
            short_name=framework_identifier
        )
    ...
    def upsert_template_document(self, content_reader, entry, user):
        existing = (
            TemplateDocument.objects.filter(source_path=entry["source_path"])
    ...
    def show_dry_run_summary(self, entries):
        ...
        for entry in entries[:5]:
            self.stdout.write(
                f"  - {entry['module']}/{entry['document_type']}: {entry['source_filename']}"
            )
