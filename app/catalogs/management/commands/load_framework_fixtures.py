...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--update", action="store_true", help="Update existing frameworks if they already exist"
        )
        parser.add_argument(
            "--user", type=str, help="Username of user importing the frameworks", default="system"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not framework_files:
            self.stdout.write(self.style.WARNING(f"No framework files found in {fixtures_dir}"))
            return
        ...
        if success_count > 0:
            self.stdout.write(self.style.SUCCESS("Framework fixtures loaded successfully!"))
