...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("framework_id", type=int, help="Framework ID to export")
        parser.add_argument("output_path", type=str, help="Output file path (.json or .yaml/.yml)")
        parser.add_argument(
            "--format",
            choices=["json", "yaml"],
    ...
    def handle(self, *args, **options):
        ...
        if not format_choice:
            if output_path.suffix.lower() == ".json":
                ...
                raise CommandError(
                    "Cannot determine output format from file extension. Use --format"
                )
