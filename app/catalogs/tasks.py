...
def send_bulk_assessment_reminders(assessment_ids, reminder_type="due_today"):
    ...
    try:
        ...
        assessments = ControlAssessment.objects.filter(
            id__in=assessment_ids, assigned_to__isnull=False
        ...
        return {"status": "error", "error_message": str(e), "timestamp": timezone.now().isoformat()}
...
Your Compliance Management System
        ...
        logger.info(
            f"Test reminder sent to user {user.username}: {'success' if success else 'failed'}"
        )
