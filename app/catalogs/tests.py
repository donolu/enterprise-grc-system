...
class ClauseModelTest(TenantTestCase):
    ...
    def test_get_all_subclauses(self):
        ...
        parent = Clause.objects.create(
            framework=self.framework, clause_id="1", title="Parent", description="Parent"
        )
    ...
    def test_import_xlsx_framework_creates_clauses_and_controls(self):
        ...
        temp_file = self.create_temp_xlsx_file(
            [
                ...
                [
                    ...
                    "Inspect privileged access reports.",
                    "Use PAM reports.",
                ],
            ]
        )
        ...
        try:
            call_command(
                ...
                temp_file,
                "--name",
                "Spreadsheet Security Framework",
                "--short-name",
                "SSF",
                "--framework-version",
                "2024",
                "--issuing-organization",
                "Spreadsheet Standards Body",
                "--effective-date",
                "2024-01-01",
            ...
            self.assertEqual(
                access_control.evidence_requirements, "Inspect access review evidence."
            )
...
class CatalogsAPITest(APITestCase):
    ...
    def test_control_testing_endpoint(self):
        ...
        response = self.client.post(
            f"/api/catalogs/controls/{self.control.id}/update_testing/", test_data
        )
...
class ControlAssessmentModelTest(TenantTestCase):
    ...
    def setUp(self):
        ...
        self.assigned_user = User.objects.create_user(
            username="assigned_user", email="assigned@example.com", password="testpass123"
        )
    ...
    def test_assessment_str_representation(self):
        ...
        assessment = ControlAssessment.objects.create(
            control=self.control, applicability="applicable", created_by=self.user
        )
    ...
    def test_assessment_is_overdue_property(self):
        ...
        assessment = ControlAssessment.objects.create(
            control=self.control, due_date=date.today() + timedelta(days=10), created_by=self.user
        )
    ...
    def test_assessment_days_until_due_property(self):
        ...
        # Future due date
        assessment = ControlAssessment.objects.create(
            control=self.control, due_date=date.today() + timedelta(days=5), created_by=self.user
        )
    ...
    def test_assessment_framework_info_property(self):
        ...
        assessment = ControlAssessment.objects.create(control=self.control, created_by=self.user)
    ...
    def test_assessment_update_status(self):
        ...
        assessment = ControlAssessment.objects.create(
            control=self.control, status="pending", created_by=self.user
        )
    ...
    def test_assessment_unique_constraint(self):
        ...
        ControlAssessment.objects.create(control=self.control, created_by=self.user)

        with self.assertRaises(IntegrityError):
            ControlAssessment.objects.create(control=self.control, created_by=self.assigned_user)

    def test_assessment_status_transitions(self):
        """Test valid status transitions."""
        assessment = ControlAssessment.objects.create(
            control=self.control, status="not_started", created_by=self.user
        )
...
class AssessmentEvidenceModelTest(TenantTestCase):
    ...
    def test_assessment_evidence_str_representation(self):
        ...
        assessment_evidence = AssessmentEvidence.objects.create(
            assessment=self.assessment, evidence=self.evidence, linked_by=self.user
        )
...
class ControlAssessmentAPITest(APITestCase):
    ...
    def test_assessment_update_endpoint(self):
        ...
        response = self.client.patch(
            f"/api/catalogs/assessments/{self.assessment.id}/", update_data
        )
    ...
    def test_assessment_status_update_endpoint(self):
        ...
        status_data = {"status": "complete", "notes": "Assessment completed successfully"}

        response = self.client.post(
            f"/api/catalogs/assessments/{self.assessment.id}/update_status/", status_data
        )
    ...
    def test_assessment_filtering(self):
        ...
        response = self.client.get(
            f"/api/catalogs/assessments/?assigned_to={self.assigned_user.id}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
...
class EvidenceManagementAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.assessment = ControlAssessment.objects.create(
            control=self.control, applicability="applicable", status="pending", created_by=self.user
        )
    ...
    def test_direct_evidence_upload(self):
        ...
        test_file = InMemoryUploadedFile(
            BytesIO(file_content), None, "test_evidence.txt", "text/plain", len(file_content), None
        )
    ...
    def test_assessment_evidence_listing(self):
        ...
        document = Document.objects.create(title="Test Evidence Doc", uploaded_by=self.user)
        ...
        response = self.client.get(f"/api/catalogs/assessments/{self.assessment.id}/evidence/")
