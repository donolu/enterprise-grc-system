...
class AssessmentReminderConfigurationTest(TestCase):
    ...
    def test_get_reminder_days_daily(self):
        ...
        config = AssessmentReminderConfiguration.objects.create(
            user=self.user, reminder_frequency="daily", advance_warning_days=5
        )
    ...
    def test_get_reminder_days_weekly(self):
        ...
        config = AssessmentReminderConfiguration.objects.create(
            user=self.user, reminder_frequency="weekly"
        )
    ...
    def test_get_reminder_days_custom(self):
        ...
        config = AssessmentReminderConfiguration.objects.create(
            user=self.user, reminder_frequency="custom", custom_reminder_days=[1, 3, 7, 14]
        )
...
class AssessmentReminderLogTest(TestCase):
    ...
    def setUp(self):
        ...
        self.clause = Clause.objects.create(
            framework=self.framework, clause_id="T.1", title="Test Clause"
        )
...
class AssessmentReminderServiceTest(TestCase):
    ...
    def test_send_individual_reminder_success(self):
        ...
        config = AssessmentReminderConfiguration.objects.create(
            user=self.user, enable_reminders=True, email_notifications=True
        )
        ...
        log = AssessmentReminderLog.objects.get(
            assessment=self.assessment, user=self.user, reminder_type="advance_warning"
        )
        self.assertTrue(log.email_sent)
    ...
    def test_send_individual_reminder_disabled(self):
        ...
        config = AssessmentReminderConfiguration.objects.create(
            user=self.user, enable_reminders=False
        )
    ...
    def test_send_weekly_digest(self):
        ...
        # Create reminder configuration
        config = AssessmentReminderConfiguration.objects.create(
            user=self.user, weekly_digest_enabled=True, email_notifications=True
        )
    ...
    def test_generate_subject_line(self):
        ...
        subject = AssessmentReminderService._generate_subject_line(self.assessment, "overdue", -5)
        ...
        subject = AssessmentReminderService._generate_subject_line(self.assessment, "due_today", 0)
...
class ReminderIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.clause = Clause.objects.create(
            framework=self.framework, clause_id="A.5.1", title="Policies for information security"
        )
    ...
    def test_end_to_end_reminder_workflow(self):
        ...
        self.assertTrue(
            AssessmentReminderLog.objects.filter(
                assessment=assessment1, user=self.user1, reminder_type="advance_warning"
            ).exists()
        )

        self.assertFalse(
            AssessmentReminderLog.objects.filter(assessment=assessment2, user=self.user2).exists()
        )
