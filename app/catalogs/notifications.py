...
from .models import ControlAssessment, AssessmentReminderConfiguration, AssessmentReminderLog
    ...
    def send_individual_reminder(assessment, user, reminder_type, days_before_due=None):
        ...
        try:
            ...
            if reminder_log_exists:
                logger.debug(
                    f"Reminder already sent for {assessment.assessment_id} to {user.username}"
                )
                return False
            ...
            html_message = render_to_string("catalogs/emails/assessment_reminder.html", context)
            text_message = render_to_string("catalogs/emails/assessment_reminder.txt", context)
            ...
            logger.info(
                f"Sent {reminder_type} reminder for {assessment.assessment_id} to {user.email}"
            )
            return bool(success)

        except Exception as e:
            logger.error(
                f"Failed to send reminder for {assessment.assessment_id} to {user.username}: {str(e)}"
            )
    ...
    def send_weekly_digest(user):
        ...
        try:
            ...
            digest_sent_today = AssessmentReminderLog.objects.filter(
                user=user, reminder_type="weekly_digest", sent_at__date=today
            ).exists()
            ...
            # Render email templates
            html_message = render_to_string("catalogs/emails/weekly_digest.html", context)
            text_message = render_to_string("catalogs/emails/weekly_digest.txt", context)
            ...
            sample_assessment = upcoming_assessments.first() or overdue_assessments.first()
    ...
    def process_daily_reminders():
        ...
        try:
            ...
            active_configs = AssessmentReminderConfiguration.objects.filter(
                enable_reminders=True, email_notifications=True
            ).select_related("user")
            ...
            logger.info(
                f"Daily reminder processing complete: {total_sent} emails sent to {results['processed_users']} users"
            )
...
class AssessmentNotificationService:
    ...
    def send_assignment_notification(assessment, assigned_by=None):
        ...
        try:
            ...
            logger.info(
                f"Sent assignment notification for {assessment.assessment_id} to {assessment.assigned_to.email}"
            )
            return bool(success)
    ...
    def send_status_change_notification(assessment, old_status, new_status, changed_by=None):
        ...
        try:
            ...
            html_message = render_to_string(
                "catalogs/emails/status_change_notification.html", context
            )
            text_message = render_to_string(
                "catalogs/emails/status_change_notification.txt", context
            )
