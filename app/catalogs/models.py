...
class Framework(models.Model):
    ...
    issuing_organization = models.CharField(
        max_length=200, help_text="Organization that issued/maintains this framework"
    )
    official_url = models.URLField(
        blank=True, null=True, help_text="Official framework documentation URL"
    )
    ...
    expiry_date = models.DateField(
        null=True, blank=True, help_text="Date when this version expires"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_frameworks"
    )
    ...
    imported_from = models.CharField(
        max_length=200, blank=True, help_text="Source of framework import (file, API, manual entry)"
    )
    import_checksum = models.CharField(
        max_length=64, blank=True, help_text="Checksum for tracking changes in imported frameworks"
    )
...
class Clause(models.Model):
    ...
    clause_id = models.CharField(
        max_length=50, help_text="Framework-specific clause identifier (e.g., CC6.1, A.8.2.1)"
    )
    ...
    is_testable = models.BooleanField(
        default=True, help_text="Whether this clause can be tested/audited"
    )

    # Implementation guidance
    implementation_guidance = models.TextField(
        blank=True, help_text="Guidance on how to implement this clause"
    )
    testing_procedures = models.TextField(
        blank=True, help_text="Procedures for testing compliance with this clause"
    )
...
class Control(models.Model):
    ...
    clauses = models.ManyToManyField(
        Clause, related_name="controls", help_text="Framework clauses this control addresses"
    )
    ...
    business_owner = models.CharField(
        max_length=200, blank=True, help_text="Business unit or department responsible"
    )

    # Implementation details
    implementation_details = models.TextField(
        blank=True, help_text="Specific implementation steps and procedures"
    )
    ...
    last_test_result = models.CharField(max_length=20, choices=EFFECTIVENESS_RATINGS, blank=True)
    ...
    evidence_requirements = models.TextField(
        blank=True, help_text="What evidence is needed to demonstrate this control's effectiveness"
    )
    ...
    remediation_plan = models.TextField(
        blank=True, help_text="Plan for addressing control deficiencies"
    )

    # Metadata
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_controls"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    change_log = models.JSONField(
        default=list, blank=True, help_text="History of changes to this control"
    )
...
class ControlEvidence(models.Model):
    ...
    external_url = models.URLField(
        blank=True, null=True, help_text="External URL for evidence (e.g., system screenshot)"
    )

    # Evidence metadata
    evidence_date = models.DateField(
        default=timezone.now, help_text="Date when evidence was created or collected"
    )
    collected_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, help_text="Person who collected this evidence"
    )

    # Validation
    is_validated = models.BooleanField(
        default=False, help_text="Whether evidence has been validated"
    )
...
class FrameworkMapping(models.Model):
    ...
    source_clause = models.ForeignKey(
        Clause, on_delete=models.CASCADE, related_name="source_mappings"
    )
    target_clause = models.ForeignKey(
        Clause, on_delete=models.CASCADE, related_name="target_mappings"
    )

    # Mapping details
    mapping_type = models.CharField(max_length=20, choices=MAPPING_TYPES)
    mapping_rationale = models.TextField(
        blank=True, help_text="Explanation of why these clauses are mapped together"
    )
    confidence_level = models.IntegerField(
        default=75, help_text="Confidence percentage (0-100) in this mapping"
    )
    ...
    verified_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="verified_mappings"
    )
...
class ControlAssessment(models.Model):
    ...
    assessment_id = models.CharField(
        max_length=100, unique=True, help_text="Unique assessment identifier"
    )
    ...
    applicability_rationale = models.TextField(
        blank=True, help_text="Explanation of why the control is/isn't applicable"
    )
    ...
    due_date = models.DateField(
        null=True, blank=True, help_text="Target completion date for this assessment"
    )
    started_date = models.DateField(
        null=True, blank=True, help_text="Date when assessment work began"
    )
    completed_date = models.DateField(
        null=True, blank=True, help_text="Date when assessment was completed"
    )

    # Assessment details
    current_state_description = models.TextField(
        blank=True, help_text="Description of the current state of control implementation"
    )
    target_state_description = models.TextField(
        blank=True, help_text="Description of the desired future state"
    )
    gap_analysis = models.TextField(
        blank=True, help_text="Analysis of gaps between current and target state"
    )

    # Implementation details
    implementation_approach = models.TextField(
        blank=True, help_text="Planned or implemented approach for addressing this control"
    )
    ...
    compliance_score = models.PositiveIntegerField(
        null=True, blank=True, help_text="Compliance score (0-100) for this control"
    )

    # Notes and comments
    assessment_notes = models.TextField(
        blank=True, help_text="General notes and observations about this assessment"
    )
    reviewer_comments = models.TextField(blank=True, help_text="Comments from the reviewer")

    # Remediation planning
    remediation_plan = models.TextField(
        blank=True, help_text="Plan for addressing gaps and improving control implementation"
    )
    remediation_due_date = models.DateField(
        null=True, blank=True, help_text="Target date for completing remediation activities"
    )
    ...
    change_log = models.JSONField(
        default=list, blank=True, help_text="History of changes to this assessment"
    )
...
class AssessmentEvidence(models.Model):
    ...
    assessment = models.ForeignKey(
        ControlAssessment, on_delete=models.CASCADE, related_name="evidence_links"
    )
    evidence = models.ForeignKey(
        ControlEvidence, on_delete=models.CASCADE, related_name="assessment_links"
    )

    # Evidence relationship to assessment
    evidence_purpose = models.CharField(
        max_length=100, blank=True, help_text="Purpose of this evidence for the assessment"
    )
    is_primary_evidence = models.BooleanField(
        default=False, help_text="Whether this is primary evidence for the assessment"
    )
...
class AssessmentReminderConfiguration(models.Model):
    ...
    enable_reminders = models.BooleanField(
        default=True, help_text="Whether to send automated reminders to this user"
    )
    advance_warning_days = models.PositiveIntegerField(
        default=7, help_text="Days before due date to send first reminder"
    )
    overdue_reminders = models.BooleanField(
        default=True, help_text="Whether to send reminders for overdue assessments"
    )
    ...
    include_assessment_details = models.BooleanField(
        default=True, help_text="Whether to include detailed assessment information in emails"
    )
    include_remediation_items = models.BooleanField(
        default=True, help_text="Whether to include remediation due dates in reminders"
    )

    # Digest settings
    daily_digest_enabled = models.BooleanField(
        default=False, help_text="Whether to send daily digest of all upcoming/overdue items"
    )
    weekly_digest_enabled = models.BooleanField(
        default=True, help_text="Whether to send weekly digest of upcoming assessments"
    )
    ...
    silence_completed_assessments = models.BooleanField(
        default=True, help_text="Stop sending reminders for completed assessments"
    )
    silence_not_applicable = models.BooleanField(
        default=True, help_text="Don't send reminders for assessments marked as not applicable"
    )
...
class AssessmentReminderLog(models.Model):
    ...
    assessment = models.ForeignKey(
        ControlAssessment, on_delete=models.CASCADE, related_name="reminder_logs"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="assessment_reminder_logs"
    )
