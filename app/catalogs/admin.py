...
class FrameworkAdmin(admin.ModelAdmin):
    list_display = [
        ...
        "status",
        "is_mandatory",
        "clause_count_display",
        "issuing_organization",
        "effective_date",
    ]
    list_filter = ["framework_type", "status", "is_mandatory", "effective_date"]
    search_fields = ["name", "short_name", "description", "issuing_organization", "external_id"]
    readonly_fields = [
        "created_at",
        "updated_at",
        "clause_count_display",
        "control_count_display",
        "import_checksum",
    ]
    fieldsets = (
        (
        ...
        ("Framework Details", {"fields": ("external_id", "issuing_organization", "official_url")}),
        (
        ...
        (
            "Import Information",
            {"fields": ("imported_from", "import_checksum"), "classes": ("collapse",)},
        ),
        (
            "Metadata",
            {
...
class ClauseAdmin(admin.ModelAdmin):
    ...
    search_fields = [
        ...
        "description",
        "framework__name",
        "framework__short_name",
    ]
    readonly_fields = [
        "created_at",
        "updated_at",
        "control_count_display",
        "full_clause_id_display",
    ]
    ...
    fieldsets = (
        ...
        ("Organization", {"fields": ("parent_clause", "sort_order")}),
        ("Properties", {"fields": ("clause_type", "criticality", "is_testable")}),
        (
            "Implementation Guidance",
            {"fields": ("implementation_guidance", "testing_procedures"), "classes": ("collapse",)},
        ),
        ("External References", {"fields": ("external_references",), "classes": ("collapse",)}),
        (
            "Metadata",
            {
...
class ControlAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("control_id", "name", "description", "clauses")}),
        ("Control Properties", {"fields": ("control_type", "automation_level", "status")}),
        ("Ownership & Responsibility", {"fields": ("control_owner", "business_owner")}),
        (
            "Implementation",
            {
        ...
        (
            "Risk & Remediation",
            {"fields": ("risk_rating", "remediation_plan"), "classes": ("collapse",)},
        ),
        ("Documentation", {"fields": ("documentation_links",), "classes": ("collapse",)}),
        ("Version Control", {"fields": ("version", "change_log"), "classes": ("collapse",)}),
        (
            "Metadata",
            {
                "fields": (
    ...
    def needs_testing_display(self, obj):
        if obj.needs_testing:
            return format_html(
                '<span style="color: red; font-weight: bold;">⚠ Needs Testing</span>'
            )
...
class ControlEvidenceAdmin(admin.ModelAdmin):
    ...
    list_filter = ["evidence_type", "is_validated", "evidence_date", "created_at"]
    search_fields = ["title", "description", "control__control_id", "control__name"]
    readonly_fields = ["created_at", "updated_at"]
    raw_id_fields = ["control", "document", "collected_by", "validated_by"]
    date_hierarchy = "evidence_date"

    fieldsets = (
        ("Basic Information", {"fields": ("control", "title", "evidence_type", "description")}),
        (
            "Evidence Source",
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .select_related("control", "collected_by", "validated_by", "document")
        )
...
class FrameworkMappingAdmin(admin.ModelAdmin):
    ...
    list_filter = [
        "mapping_type",
        "confidence_level",
        "source_clause__framework__name",
        "target_clause__framework__name",
    ]
    search_fields = [
        "source_clause__clause_id",
        "target_clause__clause_id",
        "source_clause__title",
        "target_clause__title",
        "mapping_rationale",
    ]
    ...
    fieldsets = (
        ("Mapping Details", {"fields": ("source_clause", "target_clause", "mapping_type")}),
        ("Analysis", {"fields": ("mapping_rationale", "confidence_level")}),
        ("Verification", {"fields": ("created_by", "created_at", "verified_by", "verified_at")}),
    )
...
class ControlAssessmentAdmin(admin.ModelAdmin):
    ...
    list_filter = [
        ...
        "risk_rating",
        "maturity_level",
        "control__clauses__framework__name",
    ...
    fieldsets = (
        ...
        (
            "Version Control",
            {"fields": ("version", "change_log_display"), "classes": ("collapse",)},
        ),
        (
            "Metadata",
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .select_related("control", "assigned_to", "reviewer", "remediation_owner", "created_by")
    ...
    actions = [
        "mark_as_complete",
        "mark_as_in_progress",
        "bulk_assign_due_date",
        "send_immediate_reminders",
    ...
    bulk_assign_due_date.short_description = (
        "Set due date (30 days from today) for assessments without one"
    )
    ...
    def send_immediate_reminders(self, request, queryset):
        ...
        if not assessment_ids:
            self.message_user(
                request, "No assessments with assigned users selected.", level="WARNING"
            )
            return
        ...
        try:
            ...
            self.message_user(
                request, f"Queued immediate reminders for {len(assessment_ids)} assessments."
            )
        except Exception as e:
            self.message_user(request, f"Failed to queue reminders: {str(e)}", level="ERROR")
    ...
    def send_overdue_reminders(self, request, queryset):
        ...
        if not assessment_ids:
            self.message_user(
                request, "No overdue assessments with assigned users selected.", level="WARNING"
            )
            return
        ...
        try:
            ...
            self.message_user(
                request, f"Queued overdue reminders for {len(assessment_ids)} assessments."
            )
        except Exception as e:
            self.message_user(
                request, f"Failed to queue overdue reminders: {str(e)}", level="ERROR"
            )
...
class AssessmentEvidenceAdmin(admin.ModelAdmin):
    ...
    def mark_as_primary_evidence(self, request, queryset):
        ...
        for evidence_link in queryset:
            ...
            AssessmentEvidence.objects.filter(
                assessment=evidence_link.assessment, is_primary_evidence=True
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .select_related("assessment__control", "evidence", "created_by")
        )
...
class AssessmentReminderConfigurationAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        (
            "Digest Settings",
            {"fields": ["daily_digest_enabled", "weekly_digest_enabled", "digest_day_of_week"]},
        ),
        (
            "Auto-silence Settings",
            {"fields": ["silence_completed_assessments", "silence_not_applicable"]},
        ),
        ("Metadata", {"fields": ["created_at", "updated_at"], "classes": ["collapse"]}),
    ]
    ...
    def send_immediate_digest(self, request, queryset):
        ...
        self.message_user(request, f"Sent digests to {sent_count} users, {failed_count} failed.")
...
class AssessmentReminderLogAdmin(admin.ModelAdmin):
    ...
    list_filter = ["reminder_type", "email_sent", "sent_at", "days_before_due"]
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related("user", "assessment__control")
