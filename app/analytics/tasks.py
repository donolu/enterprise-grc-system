...
def generate_analytics_report(self, report_id):
    ...
    try:
        ...
        logger.info(
            f"Starting generation of {report.report_type} report in {report.export_format} format"
        )
...
def cleanup_expired_reports():
    ...
    try:
        expired_reports = AnalyticsReport.objects.filter(
            expires_at__lt=timezone.now(), status="completed"
        )
...
def cache_analytics_metrics():
    ...
    try:
        ...
        try:
            ...
            _cache_metric(
                "compliance", "Compliance Overview", "compliance_summary", compliance_data
            )
            cached_count += 1
        except Exception as e:
        ...
        try:
            ...
            _cache_metric(
                "training", "Training Effectiveness Summary", "training_summary", training_data
            )
            cached_count += 1
        except Exception as e:
        ...
        logger.info(
            f"Successfully cached {cached_count} analytics metrics in {calculation_time:.2f}s"
        )
        return {"cached_count": cached_count, "calculation_time_seconds": calculation_time}
...
def _generate_pdf_report(report, data):
    ...
    story.append(
        Paragraph(
            f"Generated: {timezone.now().strftime('%B %d, %Y at %I:%M %p')}", styles["Normal"]
        )
    )
...
def _add_executive_pdf_content(story, data, styles):
    ...
    if "executive_dashboard" in data:
        ...
        if "risk_summary" in exec_data:
            ...
            story.append(
                Paragraph(
                    f"Total Active Risks: {risk_data.get('active_risks', 0)}", styles["Normal"]
                )
            )
            story.append(
                Paragraph(
                    f"Critical/High Risks: {risk_data.get('critical_high_risks', 0)}",
                    styles["Normal"],
                )
            )
            story.append(Spacer(1, 12))
