...
def test_analytics_services_do_not_import_cross_domain_models_directly():
    ...
    direct_model_imports = {
        ...
        if isinstance(node, ast.ImportFrom) and node.module in FORBIDDEN_DIRECT_MODEL_IMPORTS
    }
...
def test_cross_module_dashboard_services_smoke():
    ...
    assert (
        "video_engagement"
        in CrossModuleAnalyticsService.get_training_effectiveness_dashboard_data()
    )
