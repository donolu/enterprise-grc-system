...
class TenantDummyDataGenerator:
    ...
    def run(self):
        ...
        with tenant_context(self.tenant):
            ...
            try:
                ...
                print(
                    f"📊 Total Records: {len(users) + len(risk_categories) + len(risks) + len(frameworks) + len(clauses) + len(controls) + len(assessments) + len(audit_events)}"
                )
                print("=" * 60)
    ...
    def create_risk_data(self, users):
        ...
        categories_data = [
            ...
            {
                "name": "Strategic",
                "description": "Strategic planning and execution risks",
                "color": "#0284c7",
            },
            {
                "name": "Reputational",
        ...
        for cat_data in categories_data:
            category, _created = RiskCategory.objects.get_or_create(
        ...
        for i, risk_data in enumerate(risks_data, 1):
            ...
            risk, _created = Risk.objects.get_or_create(
                risk_id=risk_data["risk_id"], defaults=risk_data
            )
            created_risks.append(risk)
            print(f"   ✅ {risk.risk_id}: {risk.title}")

        print(
            f"✅ Created {len(created_categories)} risk categories and {len(created_risks)} risks"
        )
    ...
    def create_compliance_data(self, users):
        ...
        for fw_data in frameworks_data:
            ...
            framework, _created = Framework.objects.get_or_create(
                short_name=fw_data["short_name"], defaults=fw_data
            )
        ...
        for framework in created_frameworks[:2]:  # Just create clauses for first 2 frameworks
            ...
            for i, clause_data in enumerate(clauses_data):
                clause_data.update({"framework": framework, "sort_order": i + 1})
                clause, _created = Clause.objects.get_or_create(
                    framework=framework, clause_id=clause_data["clause_id"], defaults=clause_data
                )
            ...
            control_data.update(
                {
                    ...
                    "last_tested_date": timezone.now().date()
                    - timedelta(days=random.randint(15, 60)),
                }
            )
            control, _created = Control.objects.get_or_create(
                control_id=control_data["control_id"], defaults=control_data
            )
        ...
        print(
            f"✅ Created {len(created_frameworks)} frameworks, {len(created_clauses)} clauses, and {len(created_controls)} controls"
        )
    ...
    def create_audit_data(self, users):
        ...
        events_data = [
            {
                "event": "risk_created",
                "details": {"risk_id": "RISK-2024-001", "title": "Cloud Data Exposure Risk"},
            },
            {
                "event": "risk_updated",
                "details": {
            ...
            {
                "event": "user_login",
                "details": {"username": "risk_manager", "ip_address": "192.168.1.100"},
            },
            {
                "event": "framework_imported",
                "details": {"framework": "SOC 2 Type II", "clauses_count": 3},
            },
            {
                "event": "assessment_completed",
                "details": {"assessment_id": "ASS-CTRL-001", "score": 85},
            },
            {
                "event": "control_tested",
                "details": {"control_id": "CTRL-001", "result": "effective"},
            },
            {
                "event": "report_generated",
                "details": {"report_type": "risk_summary", "format": "pdf"},
            },
            {
                "event": "user_created",
                "details": {"username": "security_analyst", "role": "analyst"},
            },
            {
                "event": "policy_updated",
                "details": {"policy": "Information Security Policy", "version": "2.1"},
            },
            {
                "event": "export_data",
                "details": {"data_type": "risks", "format": "csv", "records": 12},
            },
        ]
