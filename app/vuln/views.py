...
class ScanJobViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def retry(self, request, pk=None):
        ...
        retry_job = create_scan_job(
            job.target, requested_by=request.user, schedule=job.schedule, scanner=job.scanner
        )
        schema_name = getattr(getattr(request, "tenant", None), "schema_name", None)
...
class VulnerabilityFindingViewSet(
    ...
    def create_risk(self, request, pk=None):
        ...
        if finding.risk:
            return Response(
                {"detail": "Finding is already linked to a risk.", "risk": finding.risk.id}
            )
    ...
    def create_risk_action(self, request, pk=None):
        ...
        if finding.risk_action:
            return Response(
                {
                    "detail": "Finding is already linked to a risk action.",
                    "risk_action": finding.risk_action.id,
                }
            )
        action = create_risk_action_from_finding(finding, user=request.user)
        return Response(
            {"risk_action": action.id, "action_id": action.action_id},
