...
def validate_scan_target_address(address):
    ...
    if allowed_suffixes and not any(
        ...
        raise TargetValidationError(
            "Target is outside the configured vulnerability scanning allow-list."
        )
    ...
    for ip in network:
        if (
            ...
            raise TargetValidationError(
                "Reserved, loopback and link-local targets are not allowed."
            )
        if ip.is_private and not allow_private:
            raise TargetValidationError(
                "Private network targets require VULN_ALLOW_PRIVATE_TARGETS=true."
            )
        break
    return address
...
def execute_scan_job(job_id):
    ...
    with transaction.atomic():
        ...
        job.save(
            update_fields=["status", "finished_at", "findings_count", "raw_summary", "updated_at"]
        )
        if job.schedule:
