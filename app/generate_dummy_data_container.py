...
"""Comprehensive dummy data generator for the GRC platform container.
...
# Setup Django environment - container version
...
from vendors.models import Vendor, VendorCategory, VendorContact, VendorService
from policies.models import Policy, PolicyCategory, PolicyVersion
from training.models import TrainingVideo, TrainingCategory
...
class GRCDataGenerator:
    ...
    def create_users(self):
        ...
        users_data = [
            {'username': 'admin', 'email': 'admin@company.com', 'first_name': 'System', 'last_name': 'Admin'},
    ...
    def create_risk_data(self):
        ...
        risks_data = [
            {'title': 'Cloud Data Breach Risk', 'description': 'Potential unauthorised access to sensitive data stored in cloud infrastructure.', 'category': 0, 'impact': 5, 'likelihood': 3, 'status': 'mitigating', 'treatment_strategy': 'mitigate', 'owner': 3},
            {'title': 'Third-party Vendor Security Risk', 'description': 'Risk of data exposure through third-party vendors with inadequate security controls.', 'category': 0, 'impact': 4, 'likelihood': 3, 'status': 'assessed', 'treatment_strategy': 'mitigate', 'owner': 4},
            {'title': 'Ransomware Attack Risk', 'description': 'Risk of business disruption and data loss due to ransomware attacks.', 'category': 0, 'impact': 5, 'likelihood': 2, 'status': 'identified', 'treatment_strategy': 'mitigate', 'owner': 3},
            {'title': 'Regulatory Non-compliance Risk', 'description': 'Risk of penalties due to failure to comply with GDPR, SOX regulations.', 'category': 3, 'impact': 4, 'likelihood': 2, 'status': 'under_review', 'treatment_strategy': 'mitigate', 'owner': 2},
            {'title': 'Supply Chain Disruption Risk', 'description': 'Risk of operational disruption due to critical supplier failure.', 'category': 1, 'impact': 3, 'likelihood': 3, 'status': 'mitigating', 'treatment_strategy': 'mitigate', 'owner': 4},
            {'title': 'Key Personnel Departure Risk', 'description': 'Risk of knowledge loss due to unexpected departure of key personnel.', 'category': 1, 'impact': 3, 'likelihood': 2, 'status': 'identified', 'treatment_strategy': 'accept', 'owner': 1},
            {'title': 'Market Volatility Risk', 'description': 'Risk of revenue impact due to economic downturn and market volatility.', 'category': 2, 'impact': 4, 'likelihood': 3, 'status': 'monitored', 'treatment_strategy': 'accept', 'owner': 1},
            {'title': 'Technology Obsolescence Risk', 'description': 'Risk of competitive disadvantage due to outdated technology stack.', 'category': 4, 'impact': 3, 'likelihood': 4, 'status': 'identified', 'treatment_strategy': 'mitigate', 'owner': 3},
            {'title': 'Data Privacy Violation Risk', 'description': 'Risk of privacy violations and regulatory penalties.', 'category': 3, 'impact': 4, 'likelihood': 2, 'status': 'assessed', 'treatment_strategy': 'mitigate', 'owner': 2},
            {'title': 'Physical Security Breach Risk', 'description': 'Risk of unauthorised physical access to facilities.', 'category': 0, 'impact': 2, 'likelihood': 2, 'status': 'mitigated', 'treatment_strategy': 'mitigate', 'owner': 3}
        ]
    ...
    def create_vendor_data(self):
        ...
        vendors_data = [
            {'name': 'Microsoft Corporation', 'category': 0, 'status': 'active', 'vendor_type': 'service_provider', 'risk_level': 'low', 'annual_spend': 120000.00, 'contact': {'first_name': 'John', 'last_name': 'Smith', 'email': 'john.smith@microsoft.com', 'title': 'Account Manager'}},
            {'name': 'Amazon Web Services', 'category': 0, 'status': 'active', 'vendor_type': 'service_provider', 'risk_level': 'medium', 'annual_spend': 240000.00, 'contact': {'first_name': 'Sarah', 'last_name': 'Johnson', 'email': 'sarah.j@aws.amazon.com', 'title': 'Solutions Architect'}},
            {'name': 'Deloitte Consulting', 'category': 1, 'status': 'active', 'vendor_type': 'consultant', 'risk_level': 'medium', 'annual_spend': 85000.00, 'contact': {'first_name': 'Michael', 'last_name': 'Chen', 'email': 'mchen@deloitte.com', 'title': 'Senior Consultant'}},
            {'name': 'Okta Inc.', 'category': 2, 'status': 'active', 'vendor_type': 'service_provider', 'risk_level': 'medium', 'annual_spend': 45000.00, 'contact': {'first_name': 'Jennifer', 'last_name': 'Wilson', 'email': 'jwilson@okta.com', 'title': 'Customer Success Manager'}},
            {'name': 'CrowdStrike Holdings', 'category': 2, 'status': 'active', 'vendor_type': 'service_provider', 'risk_level': 'low', 'annual_spend': 72000.00, 'contact': {'first_name': 'David', 'last_name': 'Brown', 'email': 'dbrown@crowdstrike.com', 'title': 'Security Specialist'}},
            {'name': 'JPMorgan Chase Bank', 'category': 3, 'status': 'active', 'vendor_type': 'service_provider', 'risk_level': 'low', 'annual_spend': 24000.00, 'contact': {'first_name': 'Lisa', 'last_name': 'Garcia', 'email': 'lgarcia@jpmorgan.com', 'title': 'Relationship Manager'}},
            {'name': 'Atlassian Corporation', 'category': 2, 'status': 'under_review', 'vendor_type': 'service_provider', 'risk_level': 'medium', 'annual_spend': 36000.00, 'contact': {'first_name': 'Robert', 'last_name': 'Martinez', 'email': 'rmartinez@atlassian.com', 'title': 'Account Executive'}},
            {'name': 'KPMG LLP', 'category': 1, 'status': 'approved', 'vendor_type': 'consultant', 'risk_level': 'low', 'annual_spend': 95000.00, 'contact': {'first_name': 'Amanda', 'last_name': 'Taylor', 'email': 'ataylor@kpmg.com', 'title': 'Partner'}}
        ]
    ...
    def create_policy_data(self):
        ...
        policies_data = [
            {'title': 'Information Security Policy', 'category': 0, 'policy_type': 'policy', 'status': 'approved', 'version': '2.1', 'summary': 'Updated encryption requirements'},
            {'title': 'Data Classification Standard', 'category': 0, 'policy_type': 'standard', 'status': 'approved', 'version': '1.3', 'summary': 'Added cloud storage guidelines'},
            {'title': 'Remote Work Policy', 'category': 1, 'policy_type': 'policy', 'status': 'approved', 'version': '1.5', 'summary': 'Updated VPN requirements'},
            {'title': 'Incident Response Procedure', 'category': 0, 'policy_type': 'procedure', 'status': 'approved', 'version': '3.0', 'summary': 'Major update to escalation procedures'},
            {'title': 'Vendor Risk Assessment Framework', 'category': 2, 'policy_type': 'framework', 'status': 'approved', 'version': '1.2', 'summary': 'Enhanced risk evaluation criteria'},
            {'title': 'Employee Onboarding Checklist', 'category': 1, 'policy_type': 'procedure', 'status': 'under_review', 'version': '2.0', 'summary': 'Streamlined onboarding process'},
            {'title': 'Business Continuity Plan', 'category': 3, 'policy_type': 'procedure', 'status': 'approved', 'version': '1.8', 'summary': 'Updated recovery objectives'},
            {'title': 'Code of Conduct', 'category': 1, 'policy_type': 'policy', 'status': 'approved', 'version': '4.0', 'summary': 'Annual review with updated ethics guidelines'}
        ]
    ...
    def create_training_data(self):
        ...
        videos_data = [
            {'title': 'Introduction to Cybersecurity', 'description': 'Learn the fundamentals of cybersecurity.', 'category': 0, 'difficulty': 'beginner', 'duration': 15, 'video_url': 'https://synthesia.io/embed/abc123'},
            {'title': 'Spotting Phishing Emails', 'description': 'Learn to identify common phishing techniques.', 'category': 1, 'difficulty': 'intermediate', 'duration': 22, 'video_url': 'https://synthesia.io/embed/def456'},
            {'title': 'Password Security Best Practices', 'description': 'Create strong passwords and use multi-factor authentication.', 'category': 0, 'difficulty': 'beginner', 'duration': 18, 'video_url': 'https://synthesia.io/embed/ghi789'},
            {'title': 'Data Classification Guidelines', 'description': 'Understand how to properly classify sensitive data.', 'category': 2, 'difficulty': 'intermediate', 'duration': 28, 'video_url': 'https://synthesia.io/embed/jkl012'},
            {'title': 'Social Engineering Attacks', 'description': 'Recognize and defend against social engineering tactics.', 'category': 1, 'difficulty': 'advanced', 'duration': 25, 'video_url': 'https://synthesia.io/embed/mno345'},
            {'title': 'Secure Remote Work Practices', 'description': 'Best practices for maintaining security while working remotely.', 'category': 0, 'difficulty': 'intermediate', 'duration': 20, 'video_url': 'https://synthesia.io/embed/pqr678'}
        ]
    ...
    def create_framework_data(self):
        ...
        frameworks_data = [
            {'name': 'ISO 27001:2022 Information Security Management', 'short_name': 'ISO27001', 'type': 'security', 'organization': 'International Organization for Standardization', 'version': '2022', 'status': 'active', 'mandatory': True, 'clauses': [{'id': 'A.5.1', 'title': 'Information Security Policies', 'type': 'policy'}, {'id': 'A.8.2', 'title': 'Data Classification', 'type': 'control'}]},
            {'name': 'SOC 2 Type II Service Organization Control', 'short_name': 'SOC2', 'type': 'operational', 'organization': 'American Institute of CPAs', 'version': '2017', 'status': 'active', 'mandatory': True, 'clauses': [{'id': 'CC6.1', 'title': 'Logical and Physical Access Controls', 'type': 'control'}]},
            {'name': 'NIST Cybersecurity Framework', 'short_name': 'NIST-CSF', 'type': 'security', 'organization': 'National Institute of Standards and Technology', 'version': '1.1', 'status': 'active', 'mandatory': False, 'clauses': [{'id': 'ID.AM-1', 'title': 'Physical devices and systems inventory', 'type': 'control'}]},
            {'name': 'PCI Data Security Standard', 'short_name': 'PCI-DSS', 'type': 'financial', 'organization': 'PCI Security Standards Council', 'version': '4.0', 'status': 'active', 'mandatory': True, 'clauses': [{'id': '1.1.1', 'title': 'Firewall configuration standards', 'type': 'control'}]},
            {'name': 'GDPR General Data Protection Regulation', 'short_name': 'GDPR', 'type': 'privacy', 'organization': 'European Union', 'version': '2018', 'status': 'active', 'mandatory': True, 'clauses': [{'id': 'Art.32', 'title': 'Security of processing', 'type': 'control'}]}
        ]
    ...
    def create_control_data(self):
        ...
        print('✅ Created 5 control records')
