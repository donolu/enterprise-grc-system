...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Imported {imported} assets; updated {updated}."))
    ...
    def extract_row(self, row, header_map, sheet_name):
        ...
        metadata = {
            ...
            "physical_memory_mb": self.clean_text(
                self.get_cell(row, header_map, "physical_memory_mb")
            ),
        ...
        return {
            ...
            "operating_system": self.clean_text(self.get_cell(row, header_map, "operating_system"))[
                :255
            ],
            "version": self.clean_text(self.get_cell(row, header_map, "version"))[:100],
            "last_seen_at": self.parse_datetime_value(
                self.get_cell(row, header_map, "last_seen_at")
            ),
    ...
    def import_entries(self, entries, file_path, user):
        ...
        for entry in entries:
            ...
            _asset, created = Asset.objects.update_or_create(
    ...
    def show_dry_run_summary(self, entries):
        ...
        for entry in entries[:5]:
            self.stdout.write(f"  - {entry['source_sheet']}: {entry['asset_id']} {entry['name']}")
