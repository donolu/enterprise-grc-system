...
class AssetAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identification", {"fields": ("asset_id", "name", "asset_type", "description")}),
        ("Classification", {"fields": ("classification", "criticality", "lifecycle_status")}),
        ("Ownership", {"fields": ("owner", "owner_name", "custodian", "location")}),
        (
            "Technical Details",
            {
                "fields": (
    ...
    def control_count(self, obj):
        ...
        if count:
            url = (
                reverse("admin:catalogs_control_changelist") + f"?linked_assets__id__exact={obj.id}"
            )
