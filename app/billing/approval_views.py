...
class LimitOverrideRequestViewSet(viewsets.ModelViewSet):
        ...
        try:
            ...
            with schema_context("public"):
                ...
                if limit_type == "max_users":
                    ...
                    return Response(
                        {"error": "Invalid limit type"}, status=status.HTTP_400_BAD_REQUEST
                    )
    ...
    def reject(self, request, pk=None):
        ...
        if not reason:
            return Response(
                {"error": "Rejection reason is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        with schema_context("public"):
            ...
            return Response(
                {"error": "Failed to reject request"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def apply_override(self, request, pk=None):
        ...
        with schema_context("public"):
            ...
            return Response(
                {"error": "Failed to apply override"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        ...
        return Response(
            {
                ...
                "needs_first_approval": pending_requests.filter(
                    first_approved_at__isnull=True
                ).count(),
                "needs_second_approval": pending_requests.filter(
                    first_approved_at__isnull=False, second_approved_at__isnull=True
                ).count(),
            }
        )
    ...
    def approved_pending_application(self, request):
        ...
        if not request.user.is_staff:
            return Response(
                {"error": "Only administrators can view this"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        return Response(
            {"approved_requests": serializer.data, "total_count": approved_requests.count()}
        )
