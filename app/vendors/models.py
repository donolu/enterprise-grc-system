...
class RegionalConfig(models.Model):
    ...
    region_code = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r"^[A-Z]{2,10}$", message="Enter a valid region code (e.g., US, EU, APAC)"
            )
        ],
    ...
    compliance_standards = models.JSONField(
        default=list, help_text="List of compliance standards applicable to this region"
    )

    # Validation Rules
    validation_rules = models.JSONField(
        default=dict, help_text="Custom validation rules for fields in this region"
    )

    # Data Processing Requirements
    data_processing_requirements = models.JSONField(
        default=dict, help_text="Specific data processing and privacy requirements"
    )

    # Contract Requirements
    contract_requirements = models.JSONField(
        default=dict, help_text="Region-specific contract terms and requirements"
    )
...
class VendorCategory(models.Model):
    ...
    color_code = models.CharField(
        ...
        default="#6c757d",
        validators=[
            RegexValidator(regex=r"^#[0-9A-Fa-f]{6}$", message="Enter a valid hex color code")
        ],
...
class Vendor(models.Model):
    ...
    business_description = models.TextField(
        blank=True, help_text="Description of vendor's business and services provided"
    )
    website = models.URLField(blank=True)
    tax_id = models.CharField(
        max_length=50, blank=True, help_text="Tax ID, EIN, or equivalent business identifier"
    )
    duns_number = models.CharField(
        max_length=20, blank=True, help_text="D-U-N-S Number for business identification"
    )
    ...
    payment_terms = models.CharField(
        max_length=50, blank=True, help_text="Standard payment terms (e.g., Net 30, Net 60)"
    )
    ...
    custom_fields = models.JSONField(
        default=dict, blank=True, help_text="Region-specific custom fields and their values"
    )

    # Compliance and Certifications
    certifications = models.JSONField(
        default=list, blank=True, help_text="List of relevant certifications (ISO, SOC, etc.)"
    )
    compliance_status = models.JSONField(
        default=dict, blank=True, help_text="Compliance status for various requirements"
    )
    ...
    relationship_start_date = models.DateField(
        null=True, blank=True, help_text="Date relationship with vendor began"
    )
    ...
    renewal_notice_days = models.IntegerField(
        default=90, help_text="Days notice required for contract renewal/termination"
    )
    ...
    notes = models.TextField(blank=True, help_text="Additional notes and comments about the vendor")
    tags = models.JSONField(
        default=list, blank=True, help_text="Tags for categorization and filtering"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_vendors"
    )
...
class VendorService(models.Model):
    ...
    service_code = models.CharField(
        max_length=50, blank=True, help_text="Internal service code or SKU"
    )
    ...
    category = models.CharField(max_length=30, choices=SERVICE_CATEGORY_CHOICES, default="other")
    ...
    risk_assessment_required = models.BooleanField(
        default=True, help_text="Service requires formal risk assessment"
    )
...
class VendorNote(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="vendor_notes"
    )
...
class VendorTask(models.Model):
    ...
    task_type = models.CharField(
        max_length=30, choices=TASK_TYPE_CHOICES, help_text="Type of vendor-related task"
    )
    title = models.CharField(max_length=200, help_text="Descriptive title for the task")
    description = models.TextField(
        blank=True, help_text="Detailed description of the task requirements"
    )

    # Task Scheduling
    due_date = models.DateField(help_text="Date when the task is due for completion")
    start_date = models.DateField(
        null=True, blank=True, help_text="Optional start date for the task"
    )
    completed_date = models.DateTimeField(
        null=True, blank=True, help_text="Actual completion timestamp"
    )
    ...
    last_reminder_sent = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp of last reminder sent"
    )
    reminder_recipients = models.JSONField(
        default=list, blank=True, help_text="Additional email addresses for reminders"
    )

    # Task Context and Integration
    related_contract_number = models.CharField(
        max_length=100, blank=True, help_text="Related contract number if applicable"
    )
    ...
    completion_notes = models.TextField(
        blank=True, help_text="Notes about task completion, results, or outcomes"
    )
    attachments = models.JSONField(
        default=list, blank=True, help_text="References to attached documents or evidence"
    )

    # Recurrence Configuration
    is_recurring = models.BooleanField(
        default=False, help_text="Whether this task repeats on a schedule"
    )
    ...
    auto_generated = models.BooleanField(
        default=False, help_text="Whether this task was automatically generated from contract dates"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and self.is_recurring
            and not self.parent_task
            and self.recurrence_pattern
        ):
