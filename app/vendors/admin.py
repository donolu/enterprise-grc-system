...
class RegionalConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Field Requirements",
            {"fields": ("required_fields", "custom_fields"), "classes": ("collapse",)},
        ),
        (
            "Compliance & Standards",
            {"fields": ("compliance_standards", "validation_rules"), "classes": ("collapse",)},
        ),
        (
            "Data Processing",
            {
...
class VendorCategoryAdmin(admin.ModelAdmin):
    ...
    def colored_name(self, obj):
        ...
        return format_html(
            '<span style="color: {}; font-weight: bold;">● {}</span>', obj.color_code, obj.name
        )
...
class VendorAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            ...
            {
                "fields": (("primary_region", "operating_regions"), "custom_fields"),
                "classes": ("collapse",),
            },
        ),
        (
        ...
        (
            ...
            {"fields": (("assigned_to", "relationship_start_date"), "notes", "tags")},
        ),
        (
            "Metadata",
    ...
    def contract_status(self, obj):
        ...
        if days is None:
            ...
            return format_html(
                '<span style="color: #fd7e14; font-weight: bold;">Expires in {} days</span>', days
            )
        else:
    ...
    def performance_indicator(self, obj):
        ...
        return format_html(
            '<span style="color: {}; font-weight: bold;">{} {:.1f}%</span>', color, icon, score
        )
...
class VendorServiceAdmin(admin.ModelAdmin):
    ...
    def colored_category(self, obj):
        ...
        return format_html('<span style="font-weight: bold;">{}</span>', obj.get_category_display())
...
class VendorNoteAdmin(admin.ModelAdmin):
    ...
    def internal_indicator(self, obj):
        ...
        if obj.is_internal:
            return format_html(
                '<span style="color: #fd7e14; font-weight: bold;">🔒 Internal</span>'
            )
        return "-"
...
class VendorTaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            "Task Information",
            {"fields": ("task_id", "vendor", "task_type", "title", "description")},
        ),
        (
            "Scheduling",
            {"fields": ("due_date", "start_date", "completed_date", "status", "priority")},
        ),
        ("Assignment", {"fields": ("assigned_to", "created_by")}),
        (
            "Reminders",
            {
                "fields": ("reminder_days", "last_reminder_sent", "reminder_recipients"),
                "classes": ("collapse",),
            },
        ),
        (
            "Integration",
            {"fields": ("related_contract_number", "service_reference"), "classes": ("collapse",)},
        ),
        (
            "Completion & Results",
            {"fields": ("completion_notes", "attachments"), "classes": ("collapse",)},
        ),
        (
            "Recurrence",
            {
    ...
    def vendor_link(self, obj):
        ...
        return format_html(
            '<a href="{}" style="text-decoration: none;">{}</a>', url, obj.vendor.name
        )
    ...
    def colored_priority(self, obj):
        ...
        icons = {"low": "🟢", "medium": "🟡", "high": "🟠", "urgent": "🔴", "critical": "🚨"}
    ...
    def days_until_due_display(self, obj):
        ...
        if days < 0:
            return format_html(
                '<span style="color: #dc3545; font-weight: bold;">{} days overdue</span>', abs(days)
            )
        elif days == 0:
            return format_html('<span style="color: #fd7e14; font-weight: bold;">Due today</span>')
        elif days <= 7:
    ...
    def send_reminders(self, request, queryset):
        ...
        self.message_user(
            request, f"Sent {results.get('sent', 0)} reminders, {results.get('failed', 0)} failed."
        )
