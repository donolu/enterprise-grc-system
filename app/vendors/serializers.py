...
class VendorCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get("renewal_notice_days") is not None and data["renewal_notice_days"] < 0:
            raise serializers.ValidationError("Renewal notice days must be a positive number.")
...
class VendorSummarySerializer(serializers.Serializer):
    ...
    average_performance_score = serializers.DecimalField(
        ...
        if value:
            ...
            for field in required_fields:
                if field not in value:
                    raise serializers.ValidationError(
                        f"Recurrence pattern must include '{field}' field."
                    )
...
class VendorTaskBulkActionSerializer(serializers.Serializer):
    ...
    task_ids = serializers.ListField(child=serializers.IntegerField(), min_length=1, max_length=100)
    ...
    def validate(self, data):
        ...
        if action == "update_status" and not data.get("status"):
            raise serializers.ValidationError(
                {"status": "Status is required for status update action."}
            )

        if action == "assign_user" and not data.get("assigned_to"):
            raise serializers.ValidationError(
                {"assigned_to": "User is required for assign user action."}
            )

        if action == "update_priority" and not data.get("priority"):
            raise serializers.ValidationError(
                {"priority": "Priority is required for priority update action."}
            )
...
class VendorTaskSummarySerializer(serializers.Serializer):
    ...
    # Performance metrics
    average_completion_time = serializers.DecimalField(
        max_digits=10, decimal_places=2, allow_null=True
    )
    on_time_completion_rate = serializers.DecimalField(
...
class VendorTaskReminderSerializer(serializers.Serializer):
    ...
    force_send = serializers.BooleanField(
        default=False, help_text="Send reminders even if not scheduled for today"
    )
