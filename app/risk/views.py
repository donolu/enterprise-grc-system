...
from .serializers import (
    ...
    BulkRiskCreateSerializer,
    RiskSummarySerializer,
    RiskCategorySerializer,
    RiskMatrixSerializer,
    RiskNoteSerializer,
    RiskActionListSerializer,
    RiskActionDetailSerializer,
    RiskActionCreateUpdateSerializer,
    RiskActionStatusUpdateSerializer,
    RiskActionNoteCreateSerializer,
    RiskActionNoteSerializer,
    RiskActionEvidenceCreateUpdateSerializer,
    RiskActionEvidenceSerializer,
    RiskActionBulkCreateSerializer,
    RiskActionSummarySerializer,
    RiskActionReminderConfigurationSerializer,
)
...
class RiskViewSet(viewsets.ModelViewSet):
    ...
    **Common Use Cases:**
    ...
    def get_queryset(self):
        ...
        queryset = (
            Risk.objects.select_related("category", "risk_owner", "risk_matrix", "created_by")
            .prefetch_related("notes")
            .alias(risk_score=F("impact") * F("likelihood"))
        )

        # Apply common filters
        if self.request.query_params.get("overdue_review"):
            queryset = queryset.filter(next_review_date__lt=timezone.now().date()).exclude(
    ...
    def update_status(self, request, pk=None):
        ...
        serializer = RiskStatusUpdateSerializer(data=request.data, context={"request": request})
    ...
    def bulk_create(self, request):
        ...
        serializer = BulkRiskCreateSerializer(data=request.data, context={"request": request})
    ...
    def summary(self, request):
        ...
        overdue_reviews = (
            queryset.filter(next_review_date__lt=timezone.now().date())
        ...
        by_treatment_strategy = dict(
            queryset.exclude(treatment_strategy__isnull=True)
            .exclude(treatment_strategy="")
            .values("treatment_strategy")
            .annotate(count=Count("id"))
        ...
        recent_risks = queryset.filter(created_at__date__gte=thirty_days_ago).order_by(
            "-created_at"
        )[:10]

        # High priority risks
        high_priority_risks = (
            queryset.filter(risk_level__in=["high", "critical"])
            .filter(
                status__in=["identified", "assessed", "treatment_planned", "treatment_in_progress"]
            )
    ...
    def by_category(self, request):
        ...
        categories = (
            RiskCategory.objects.annotate(
                risk_count=Count("risks", filter=Q(risks__in=queryset)),
                high_priority_count=Count(
                    "risks",
                    filter=Q(risks__in=queryset, risks__risk_level__in=["high", "critical"]),
                ),
            )
    ...
    **Common Use Cases:**
    ...
    def update_status(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            response_serializer = RiskActionDetailSerializer(
                updated_action, context={"request": request}
            )
            return Response(response_serializer.data)
    ...
    def add_note(self, request, pk=None):
        ...
        serializer = RiskActionNoteCreateSerializer(
            data=request.data, context={"request": request, "action": action}
        )
        ...
        if serializer.is_valid():
            ...
            response_serializer = RiskActionNoteSerializer(note, context={"request": request})
    ...
    def bulk_create(self, request):
        ...
        serializer = RiskActionBulkCreateSerializer(data=request.data, context={"request": request})
        ...
        if serializer.is_valid():
            ...
            created_serializer = RiskActionListSerializer(
                created_actions, many=True, context={"request": request}
            )
    ...
    def summary(self, request):
        ...
        overdue_count = queryset.filter(
            due_date__lt=today, status__in=["pending", "in_progress", "deferred"]
        ).count()
        ...
        high_priority_pending = queryset.filter(
            priority__in=["high", "critical"], status__in=["pending", "in_progress", "deferred"]
        ).count()
        ...
        overdue_serializer = RiskActionListSerializer(
            overdue_actions, many=True, context={"request": request}
        )
        due_soon_serializer = RiskActionListSerializer(
            due_soon_actions, many=True, context={"request": request}
        )
        high_priority_serializer = RiskActionListSerializer(
            high_priority_actions, many=True, context={"request": request}
        )
    ...
    def by_risk(self, request):
        ...
        risks_with_actions = (
            Risk.objects.filter(actions__in=queryset)
            .annotate(
                action_count=Count("actions", filter=Q(actions__in=queryset)),
                completed_count=Count(
                    "actions", filter=Q(actions__in=queryset, actions__status="completed")
                ),
            )
            .order_by("risk_id")
        )
        ...
        for risk in risks_with_actions:
            ...
            action_serializer = RiskActionListSerializer(
                risk_actions, many=True, context={"request": request}
            )
                ...
                examples=[
                    OpenApiExample(
                        ...
                        value={
                            ...
                            "trend_analysis": {"creation_trend": [], "closure_trend": []},
                        },
                    )
                ],
            )
    ...
    **Dashboard Data Structure:**
    ...
    @extend_schema(
        ...
        responses={
            ...
            500: OpenApiResponse(description="Failed to generate risk overview"),
        },
        tags=["Risk Analytics"],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiTypes.OBJECT,
            500: OpenApiResponse(description="Failed to generate action overview"),
        },
        tags=["Risk Analytics"],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiTypes.OBJECT,
            500: OpenApiResponse(description="Failed to generate heat map"),
        },
        tags=["Risk Analytics"],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiTypes.OBJECT,
            500: OpenApiResponse(description="Failed to generate progress analysis"),
        },
        tags=["Risk Analytics"],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiTypes.OBJECT,
            500: OpenApiResponse(description="Failed to generate executive summary"),
        },
        tags=["Risk Analytics"],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiTypes.OBJECT,
            500: OpenApiResponse(description="Failed to generate control integration analysis"),
        },
        tags=["Risk Analytics"],
    )
    @action(detail=False, methods=["get"])
