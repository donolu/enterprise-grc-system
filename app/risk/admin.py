...
class RiskCategoryAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).annotate(risk_count=Count("risks"))
...
class RiskMatrixAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ("Basic Information", {"fields": ["name", "description", "is_default"]}),
        (
    ...
    def get_queryset(self, request):
        ...
        return (
            ...
            .annotate(risk_count=Count("risk"))
        )
...
class RiskAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        (
            "Risk Identification",
            {"fields": ["risk_id", "title", "description", "category", "identified_date"]},
        ),
        (
            "Risk Assessment",
            {
        ...
        (
            ...
            {
                "fields": ["potential_impact_description", "current_controls"],
                "classes": ["collapse"],
            },
        ),
        ("Dates", {"fields": ["closed_date", "created_at", "updated_at"], "classes": ["collapse"]}),
    ...
    def get_queryset(self, request):
        ...
        return (
            super()
            .get_queryset(request)
            .select_related("category", "risk_owner", "risk_matrix", "created_by")
...
class RiskNoteAdmin(admin.ModelAdmin):
    ...
    list_display = ["risk_display", "note_type", "note_preview", "created_by_display", "created_at"]
    ...
    def get_queryset(self, request):
        """Optimize queryset."""
        return super().get_queryset(request).select_related("risk", "created_by")
...
class RiskActionAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        (
            "Action Information",
            {"fields": ["action_id", "title", "description", "action_type", "risk"]},
        ),
        (
            "Assignment & Priority",
            {"fields": ["assigned_to", "priority", "status", "progress_percentage"]},
        ),
        (
            "Scheduling",
            {
        ...
        (
            "Cost & Effort",
            {
                "fields": ["estimated_cost", "actual_cost", "estimated_effort_hours"],
                "classes": ["collapse"],
            },
        ),
        ("Requirements", {"fields": ["success_criteria", "dependencies"], "classes": ["collapse"]}),
        (
            "Metadata",
            {"fields": ["created_at", "updated_at", "created_by"], "classes": ["collapse"]},
        ),
    ...
    def get_queryset(self, request):
        ...
        return (
            super()
            .get_queryset(request)
            .select_related("risk", "assigned_to", "created_by")
    ...
    def risk_display(self, obj):
        ...
        if obj.risk:
            ...
            return format_html(
                '<a href="{}" style="color: {};">{}</a>', url, color, obj.risk.risk_id
            )
        return "No Risk"
    ...
    def due_date_display(self, obj):
        ...
        if obj.is_overdue:
            return format_html(
                '<span style="color: #DC2626; font-weight: bold;">{} (Overdue)</span>', obj.due_date
            )
        elif obj.is_due_soon:
            return format_html('<span style="color: #F59E0B;">{} (Due Soon)</span>', obj.due_date)
        else:
            return obj.due_date
    ...
    def days_until_due_display(self, obj):
        ...
        if days < 0:
            return format_html(
                '<span style="color: #DC2626; font-weight: bold;">{} days overdue</span>', abs(days)
            )
        elif days == 0:
            return format_html('<span style="color: #F59E0B; font-weight: bold;">Due today</span>')
        else:
    ...
    def send_reminder_notifications(self, request, queryset):
        ...
        self.message_user(request, f"Queued {sent_count} reminder notifications for sending.")
...
class RiskActionNoteAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """Optimize queryset."""
        return super().get_queryset(request).select_related("action", "action__risk", "created_by")

    def action_display(self, obj):
        """Display linked action."""
        url = reverse("admin:risk_riskaction_change", args=[obj.action.pk])
        return format_html('<a href="{}">{}</a>', url, obj.action.action_id)
...
class RiskActionEvidenceAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ["uploaded_by", "created_at", "validated_by", "validated_at"]

    fieldsets = [
        ("Evidence Information", {"fields": ["title", "description", "evidence_type", "action"]}),
        ("Files & Links", {"fields": ["file", "external_link"]}),
        (
            "Validation",
        ...
        (
            "Metadata",
            {"fields": ["evidence_date", "uploaded_by", "created_at"], "classes": ["collapse"]},
        ),
    ]
    ...
    def get_queryset(self, request):
        ...
        return (
            super()
            .get_queryset(request)
            .select_related("action", "action__risk", "uploaded_by", "validated_by")
        )
    ...
    def mark_as_unvalidated(self, request, queryset):
        ...
        count = queryset.update(
            is_validated=False, validated_by=None, validated_at=None, validation_notes=""
        )
...
class RiskActionReminderConfigurationAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ("Notification Preferences", {"fields": ["email_notifications", "overdue_reminders"]}),
        ("Weekly Digest", {"fields": ["weekly_digest_enabled", "weekly_digest_day"]}),
        ("Auto-silence Options", {"fields": ["silence_completed", "silence_cancelled"]}),
    ...
    def send_test_reminders(self, request, queryset):
        ...
        self.message_user(request, f"Queued {sent_count} test reminders for sending.")
