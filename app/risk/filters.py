...
class RiskFilter(django_filters.FilterSet):
    ...
    risk_owner = django_filters.NumberFilter(
        field_name="risk_owner", help_text="Filter by risk owner user ID."
    )

    # Risk score range filtering
    risk_score_min = django_filters.NumberFilter(
        method="filter_risk_score_min", help_text="Filter risks with risk score >= this value."
    )

    risk_score_max = django_filters.NumberFilter(
        method="filter_risk_score_max", help_text="Filter risks with risk score <= this value."
    )
    ...
    overdue_review = django_filters.BooleanFilter(
        method="filter_overdue_review", help_text="Filter risks that are overdue for review."
    )
    ...
    high_priority = django_filters.BooleanFilter(
        method="filter_high_priority", help_text="Filter only high and critical priority risks."
    )
    ...
    my_risks = django_filters.BooleanFilter(
        method="filter_my_risks", help_text="Filter risks owned by the current user."
    )

    # Text search across multiple fields
    search = django_filters.CharFilter(
        method="filter_search", help_text="Search across risk ID, title, and description."
    )
    ...
    def filter_risk_score_min(self, queryset, name, value):
        ...
        if value is not None:
            ...
            return queryset.extra(where=["impact * likelihood >= %s"], params=[value])
        return queryset
    ...
    def filter_overdue_review(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.filter(next_review_date__lt=today).exclude(
...
class RiskActionFilter(django_filters.FilterSet):
    ...
    overdue = django_filters.BooleanFilter(
        method="filter_overdue", help_text="Filter actions that are overdue."
    )

    due_soon = django_filters.BooleanFilter(
        method="filter_due_soon", help_text="Filter actions due within the next 7 days."
    )
    ...
    high_priority = django_filters.BooleanFilter(
        method="filter_high_priority", help_text="Filter only high and critical priority actions."
    )

    assigned_to_me = django_filters.BooleanFilter(
        method="filter_assigned_to_me", help_text="Filter actions assigned to the current user."
    )

    has_evidence = django_filters.BooleanFilter(
        method="filter_has_evidence", help_text="Filter actions that have evidence uploaded."
    )

    # Text search across multiple fields
    search = django_filters.CharFilter(
        method="filter_search", help_text="Search across action ID, title, and description."
    )
