...
class RiskCategory(models.Model):
    ...
    color = models.CharField(
        max_length=7, default="#6B7280", help_text="Hex color code for UI display"
    )
...
class RiskMatrix(models.Model):
    ...
    impact_levels = models.PositiveIntegerField(
        default=5, validators=[MinValueValidator(3), MaxValueValidator(7)]
    )
    likelihood_levels = models.PositiveIntegerField(
        default=5, validators=[MinValueValidator(3), MaxValueValidator(7)]
    )
    ...
    matrix_config = models.JSONField(
        default=dict, help_text="Matrix configuration mapping impact x likelihood to risk levels"
    )

    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_risk_matrices"
    )
...
class Risk(models.Model):
    ...
    category = models.ForeignKey(
        RiskCategory, on_delete=models.SET_NULL, null=True, blank=True, related_name="risks"
    )
    ...
    risk_level = models.CharField(
        max_length=20, choices=RISK_LEVELS, help_text="Calculated risk level"
    )
    ...
    treatment_description = models.TextField(
        blank=True, help_text="Description of the treatment approach"
    )
    ...
    # Additional context
    potential_impact_description = models.TextField(
        blank=True, help_text="Detailed description of potential impact"
    )
    current_controls = models.TextField(
        blank=True, help_text="Existing controls or mitigations in place"
    )
    ...
    updated_at = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_risks"
    )
...
class RiskNote(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="risk_notes"
    )
...
class RiskAction(models.Model):
    ...
    estimated_effort_hours = models.PositiveIntegerField(
        null=True, blank=True, help_text="Estimated effort in hours"
    )

    # Additional context
    success_criteria = models.TextField(
        blank=True, help_text="Specific criteria that define successful completion"
    )
    dependencies = models.TextField(
        blank=True, help_text="Dependencies or prerequisites for this action"
    )
    ...
    updated_at = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_risk_actions"
    )
...
class RiskActionNote(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="risk_action_notes"
    )
...
class RiskActionEvidence(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="uploaded_risk_action_evidence"
    )
...
class RiskActionReminderConfiguration(models.Model):
    ...
    advance_warning_days = models.PositiveIntegerField(
        default=7, help_text="Days before due date to send advance warning"
    )
    ...
    overdue_reminders = models.BooleanField(
        default=True, help_text="Send reminders for overdue actions"
    )
    weekly_digest_enabled = models.BooleanField(
        default=True, help_text="Send weekly summary digest"
    )
    ...
    silence_completed = models.BooleanField(
        default=True, help_text="Stop sending reminders for completed actions"
    )
    silence_cancelled = models.BooleanField(
        default=True, help_text="Stop sending reminders for cancelled actions"
    )
...
class RiskActionReminderLog(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="risk_action_reminder_logs"
    )
    ...
    days_before_due = models.IntegerField(
        null=True, blank=True, help_text="Days before due date when sent (negative for overdue)"
    )
