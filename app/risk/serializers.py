...
class RiskCategorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RiskCategory
        fields = ["id", "name", "description", "color", "risk_count", "created_at", "updated_at"]
        ...
        for impact in range(1, impact_levels + 1):
            ...
            if impact_str not in value:
                raise serializers.ValidationError(
                    f"Missing impact level {impact} in matrix configuration"
                )
            ...
            for likelihood in range(1, likelihood_levels + 1):
                ...
                if likelihood_str not in value[impact_str]:
                    raise serializers.ValidationError(
                        f"Missing likelihood level {likelihood} for impact {impact}"
                    )

                level = value[impact_str][likelihood_str]
                if level not in ["low", "medium", "high", "critical"]:
                    raise serializers.ValidationError(
                        f"Invalid risk level '{level}' at impact {impact}, likelihood {likelihood}"
                    )
...
class RiskNoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RiskNote
        fields = ["id", "note", "note_type", "created_at", "created_by", "created_by_name"]
...
class RiskDetailSerializer(serializers.ModelSerializer):
    ...
    risk_level_display = serializers.CharField(source="get_risk_level_display", read_only=True)
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    treatment_strategy_display = serializers.CharField(
        source="get_treatment_strategy_display", read_only=True
    )
    category_name = serializers.CharField(source="category.name", read_only=True)
    risk_owner_name = serializers.CharField(source="risk_owner.get_full_name", read_only=True)
    created_by_name = serializers.CharField(source="created_by.get_full_name", read_only=True)
...
class RiskCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            status in ["treatment_planned", "treatment_in_progress", "mitigated"]
            and not treatment_strategy
        ):
            raise serializers.ValidationError(
...
class BulkRiskCreateSerializer(serializers.Serializer):
    ...
    category = serializers.PrimaryKeyRelatedField(
        queryset=RiskCategory.objects.all(), required=False, allow_null=True
    )
    risk_owner = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.all(), required=False, allow_null=True
    )
    risk_matrix = serializers.PrimaryKeyRelatedField(
        queryset=RiskMatrix.objects.all(), required=False, allow_null=True
    )
            ...
            for i, risk_data in enumerate(self.validated_data["risks"]):
                try:
                    risk = Risk.objects.create(
                        ...
                        likelihood=risk_data.get(
                            "likelihood", self.validated_data["default_likelihood"]
                        ),
                        next_review_date=self.validated_data.get("next_review_date"),
                        potential_impact_description=risk_data.get(
                            "potential_impact_description", ""
                        ),
...
class RiskActionEvidenceCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RiskActionEvidence
        fields = ["title", "description", "evidence_type", "file", "external_link", "evidence_date"]

    def validate(self, data):
        """Validate that either file or external link is provided."""
        if not data.get("file") and not data.get("external_link"):
            raise serializers.ValidationError("Either file or external link must be provided.")
        return data
...
class RiskActionBulkCreateSerializer(serializers.Serializer):
    ...
    default_priority = serializers.ChoiceField(
        choices=RiskAction.PRIORITY_CHOICES, default="medium"
    )
    default_action_type = serializers.ChoiceField(choices=RiskAction.ACTION_TYPES, default="other")
                ...
                try:
                    action = RiskAction.objects.create(
                        ...
                        action_type=action_data.get(
                            "action_type", self.validated_data["default_action_type"]
                        ),
                        assigned_to=action_data.get("assigned_to")
                        or self.validated_data.get("assigned_to"),
                        priority=action_data.get(
                            "priority", self.validated_data["default_priority"]
                        ),
