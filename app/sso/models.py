...
class SSOProvider(models.Model):
    ...
    enforce_sso = models.BooleanField(
        default=False, help_text="Disable password authentication when SSO is available"
    )
    ...
    jit_default_role = models.CharField(
        max_length=50, default="user", help_text="Default role for JIT-provisioned users"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_sso_providers"
    )
...
class SAMLProvider(models.Model):
    ...
    sso_provider = models.OneToOneField(
        SSOProvider, on_delete=models.CASCADE, related_name="saml_config"
    )

    # Identity Provider settings
    idp_sso_url = models.URLField(help_text="IdP Single Sign-On URL")
    idp_sls_url = models.URLField(blank=True, help_text="IdP Single Logout Service URL (optional)")
    idp_x509_cert = models.TextField(help_text="IdP X.509 Certificate (Base64 encoded)")
    ...
    signature_algorithm = models.CharField(
        max_length=20, choices=SIGNATURE_ALGORITHMS, default="RSA_SHA256"
    )
    digest_algorithm = models.CharField(max_length=10, choices=DIGEST_ALGORITHMS, default="SHA256")
    ...
    sp_private_key = models.TextField(blank=True, help_text="SP Private Key for signing (optional)")
...
class OAuthProvider(models.Model):
    ...
    sso_provider = models.OneToOneField(
        SSOProvider, on_delete=models.CASCADE, related_name="oauth_config"
    )
    ...
    jwks_url = models.URLField(blank=True, help_text="JSON Web Key Set URL (for OpenID Connect)")
    discovery_url = models.URLField(blank=True, help_text="OpenID Connect Discovery URL (optional)")
    ...
    # OAuth settings
    grant_type = models.CharField(max_length=20, choices=GRANT_TYPES, default="authorization_code")
    scope = models.CharField(
        max_length=500, default="openid profile email", help_text="Space-separated OAuth scopes"
    )

    # OpenID Connect settings
    use_oidc = models.BooleanField(default=True, help_text="Use OpenID Connect features")
...
class AttributeMapping(models.Model):
    ...
    sso_provider = models.ForeignKey(
        SSOProvider, on_delete=models.CASCADE, related_name="attribute_mappings"
    )

    # Mapping configuration
    sso_attribute = models.CharField(max_length=100, help_text="Attribute name from SSO provider")
    user_field = models.CharField(
        max_length=50, choices=USER_FIELDS, help_text="User model field to map to"
    )

    # Transformation settings
    is_required = models.BooleanField(default=False)
    default_value = models.CharField(
        max_length=255, blank=True, help_text="Default value if attribute not provided"
    )
...
class SSOAuditLog(models.Model):
    ...
    sso_session = models.ForeignKey(SSOSession, on_delete=models.SET_NULL, null=True, blank=True)
