...
class SSOProviderAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ("Basic Configuration", {"fields": ["tenant", "name", "provider_type", "provider_name"]}),
        ("Status & Settings", {"fields": ["is_active", "is_primary", "enforce_sso"]}),
        ("JIT Provisioning", {"fields": ["enable_jit_provisioning", "jit_default_role"]}),
        ("Advanced", {"fields": ["entity_id"], "classes": ["collapse"]}),
        (
            "Metadata",
            {"fields": ["id", "created_at", "updated_at", "created_by"], "classes": ["collapse"]},
        ),
    ]
    ...
    def metadata_view(self, request, provider_id):
        ...
        try:
            ...
            if provider.provider_type == "saml":
                ...
                if metadata_xml:
                    response = HttpResponse(metadata_xml, content_type="application/xml")
                    response["Content-Disposition"] = (
                        f'attachment; filename="{provider.name}_metadata.xml"'
                    )
                    return response
    ...
    def test_view(self, request, provider_id):
        ...
        try:
            ...
            if errors:
                self.message_user(
                    request, f"Configuration errors found: {'; '.join(errors)}", level="error"
                )
            else:
                self.message_user(request, "Configuration validation passed", level="success")

        except SSOProvider.DoesNotExist:
            self.message_user(request, "Provider not found", level="error")

        return HttpResponseRedirect(reverse("admin:sso_ssoprovider_change", args=[provider_id]))
    ...
    def test_configuration(self, request, queryset):
        ...
        if results:
            self.message_user(
                request, f"Configuration errors: {'; '.join(results)}", level="warning"
            )
        else:
            self.message_user(
                request, f"All {queryset.count()} configurations are valid", level="success"
            )
...
class AttributeMappingAdmin(admin.ModelAdmin):
    list_display = ["sso_provider", "sso_attribute", "user_field", "is_required", "default_value"]
    list_filter = ["user_field", "is_required", "sso_provider__provider_type"]
    search_fields = ["sso_attribute", "sso_provider__name"]

    fieldsets = [
        (None, {"fields": ["sso_provider", "sso_attribute", "user_field"]}),
        ("Configuration", {"fields": ["is_required", "default_value", "transform_expression"]}),
    ]
...
class SSOSessionAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "sso_provider", "created_at", "expires_at"]
    search_fields = ["user__email", "user__username", "sso_provider__name", "ip_address"]
    readonly_fields = ["id", "created_at", "last_accessed", "sso_session_id", "sso_name_id"]
    date_hierarchy = "created_at"

    fieldsets = [
        (
            "Session Info",
            {"fields": ["id", "user", "sso_provider", "status", "sso_session_id", "sso_name_id"]},
        ),
        ("Timing", {"fields": ["created_at", "last_accessed", "expires_at"]}),
    ...
    def terminate_sessions(self, request, queryset):
        ...
        queryset.filter(status="active").update(status="terminated", last_accessed=timezone.now())
    ...
    def cleanup_expired(self, request, queryset):
        ...
        now = timezone.now()
        expired = queryset.filter(status="active", expires_at__lt=now).update(status="expired")
...
class SSOAuditLogAdmin(admin.ModelAdmin):
    ...
    list_filter = ["event_type", "success", "event_timestamp", "sso_provider__name"]
    search_fields = ["message", "user__email", "sso_provider__name", "ip_address", "request_id"]
    readonly_fields = ["id", "event_timestamp", "details", "ip_address", "user_agent", "request_id"]
    date_hierarchy = "event_timestamp"

    fieldsets = [
        (
            "Event Details",
            {"fields": ["id", "event_type", "event_timestamp", "message", "success"]},
        ),
        ("Related Objects", {"fields": ["sso_provider", "user", "sso_session"]}),
        (
            "Request Context",
            {"fields": ["ip_address", "user_agent", "request_id"], "classes": ["collapse"]},
        ),
        ("Additional Data", {"fields": ["details", "error_message"], "classes": ["collapse"]}),
    ]
