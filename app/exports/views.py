...
class AssessmentReportViewSet(viewsets.ModelViewSet):
    ...
    **Common Use Cases:**
    ...
    def get_queryset(self):
        ...
        return AssessmentReport.objects.filter(requested_by=self.request.user).select_related(
            "framework", "requested_by", "generated_file"
        )
    ...
    def generate(self, request, pk=None):
        ...
        if report.status in ["processing"]:
            return Response(
                {"error": "Report is already being generated"}, status=status.HTTP_400_BAD_REQUEST
            )

        if report.status == "completed":
            return Response({"message": "Report is already completed"}, status=status.HTTP_200_OK)
    ...
    def download(self, request, pk=None):
        ...
        if report.status != "completed" or not report.generated_file:
            return Response(
                {"error": "Report is not ready for download"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        assessment_data = [
            {
                ...
                "framework_name": ", ".join(
                    sorted(
                        {clause.framework.short_name for clause in assessment.control.clauses.all()}
                    )
                ),
            }
...
class TenantDataExportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return TenantDataExport.objects.filter(requested_by=self.request.user).select_related(
            "requested_by", "generated_file"
        )
