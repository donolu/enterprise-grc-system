...
EXPORT_COVERAGE = [
    ...
    },
    {
        ...
        "sheets": [
            ...
            {
                "model": "calendarhub.CalendarNotificationPreference",
                "worksheet": "Calendar preferences",
            },
...
class AssessmentReportGenerator:
    ...
    def _get_assessment_summary_context(self):
        ...
        if self.report.framework:
            ...
            assessments = (
                ...
                .prefetch_related("control__clauses")
                .distinct()
            )
            ...
            overdue_assessments = assessments.filter(
                status__in=["not_started", "in_progress"], due_date__lt=timezone.now().date()
            ).count()
    ...
    def _get_evidence_portfolio_context(self):
        ...
        if self.report.framework:
            evidence_links = (
                ...
                .select_related("assessment", "evidence", "assessment__control")
            ...
            evidence_links = (
                AssessmentEvidence.objects.filter(assessment_id__in=assessment_ids)
                .select_related("assessment", "evidence", "assessment__control")
                .prefetch_related("evidence__document")
            )
    ...
    def _get_compliance_gap_context(self):
        ...
        in_progress_overdue = assessments.filter(
            status="in_progress", due_date__lt=timezone.now().date()
        )
        missing_evidence = assessments.filter(status="complete", evidence_links__isnull=True)
        no_primary_evidence = assessments.filter(status="complete").exclude(
    ...
    def _calculate_risk_compliance_correlation(self):
        ...
        try:
            ...
            risk_action_completion_rate = (
                (completed_risk_actions / total_risk_actions * 100) if total_risk_actions > 0 else 0
            )

            return {
                "total_risks": total_risks,
                "risks_with_mitigation_actions": risks_with_actions,
                "risk_mitigation_coverage": (risks_with_actions / total_risks * 100)
                if total_risks > 0
                else 0,
                ...
                "integration_insights": {
                    ...
                    "comprehensive_coverage": (risks_with_actions / total_risks > 0.7)
                    if total_risks > 0
                    else False,
                },
            }
