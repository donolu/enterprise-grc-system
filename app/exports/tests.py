...
from .services import (
    ...
    TenantDataExportGenerator,
    get_export_coverage_manifest,
)
...
class AssessmentReportModelTest(TestCase):
    ...
    def test_string_representation(self):
        ...
        report = AssessmentReport.objects.create(
            report_type="detailed_assessment", title="Detailed Report", requested_by=self.user
        )
...
class AssessmentReportAPITest(ExportTenantAPITestCase):
    ...
    def test_list_assessment_reports(self):
        ...
        AssessmentReport.objects.create(
            report_type="detailed_assessment", title="Report 2", requested_by=self.user
        )
    ...
    def test_filter_reports_by_type(self):
        ...
        AssessmentReport.objects.create(
            report_type="assessment_summary", title="Summary Report", requested_by=self.user
        )
        AssessmentReport.objects.create(
            report_type="detailed_assessment", title="Detailed Report", requested_by=self.user
        )
...
class AssessmentReportGeneratorTest(TestCase):
    ...
    def test_generate_assessment_summary(self, mock_render):
        ...
        with patch("exports.services.AssessmentReportGenerator._save_pdf_document") as mock_save:
            mock_document = Document.objects.create(title="Test Report", uploaded_by=self.user)
    ...
    def test_generate_compliance_gap(self):
        ...
        not_started = ControlAssessment.objects.create(
            control=self.control, assigned_to=self.user, status="not_started"
        )
    ...
    def test_generate_unknown_report_type(self):
        ...
        report = AssessmentReport.objects.create(
            report_type="unknown_type", title="Unknown Report", requested_by=self.user
        )
    ...
    def test_generation_error_handling(self):
        ...
        with patch.object(generator.pdf_renderer, "render", side_effect=Exception("PDF error")):
