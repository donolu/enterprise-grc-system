...
class TrainingCategoryAdmin(admin.ModelAdmin):
    ...
    def videos_count(self, obj):
        ...
        return format_html(
            '<span style="color: #1890ff; font-weight: bold;">{}</span> videos', count
        )
...
class TrainingVideoAdmin(admin.ModelAdmin):
    ...
    list_filter = ["is_published", "video_provider", "difficulty_level", "category", "created_at"]
    search_fields = ["title", "description", "category__name"]
    ordering = ["-created_at"]
    readonly_fields = ["view_count", "created_at", "updated_at", "published_at"]

    fieldsets = (
        ("Video Information", {"fields": ("title", "description", "category")}),
        ("Video Source", {"fields": ("video_provider", "video_url", "video_id")}),
        ("Content Details", {"fields": ("duration_minutes", "difficulty_level")}),
    ...
    def difficulty_colored(self, obj):
        ...
        colors = {"beginner": "#52c41a", "intermediate": "#faad14", "advanced": "#f5222d"}
        return format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>',
    ...
    def publish_videos(self, request, queryset):
        ...
        self.message_user(
            request, f"{count} video(s) have been published successfully.", messages.SUCCESS
        )
    ...
    def unpublish_videos(self, request, queryset):
        ...
        self.message_user(request, f"{count} video(s) have been unpublished.", messages.SUCCESS)
...
class SecurityAwarenessCampaignAdmin(admin.ModelAdmin):
    ...
    list_filter = ["is_active", "send_frequency", "send_to_all_users", "start_date", "created_at"]
    ...
    fieldsets = (
        ("Campaign Information", {"fields": ("name", "description")}),
        ("Email Content", {"fields": ("subject_line", "email_content")}),
        (
            "Scheduling",
            {"fields": ("is_active", "send_frequency", "start_date", "end_date", "next_send_date")},
        ),
        ("Target Audience", {"fields": ("send_to_all_users", "target_users")}),
        (
            "Statistics",
            {
                "fields": (
    ...
    def status_colored(self, obj):
        ...
        if not obj.is_active:
            return format_html('<span style="color: #999; font-weight: bold;">Inactive</span>')
        elif obj.end_date and obj.end_date < now:
            return format_html('<span style="color: #666; font-weight: bold;">Ended</span>')
        elif obj.start_date > now:
            return format_html('<span style="color: #faad14; font-weight: bold;">Scheduled</span>')
        elif obj.next_send_date <= now:
            return format_html(
                '<span style="color: #f5222d; font-weight: bold;">Ready to Send</span>'
            )
        else:
            return format_html('<span style="color: #52c41a; font-weight: bold;">Active</span>')
    ...
    def target_audience(self, obj):
        ...
        if obj.send_to_all_users:
            ...
            return format_html('<span style="color: #1890ff;">All Users</span> ({})', count)
        else:
            count = obj.target_users.count()
            return format_html('<span style="color: #722ed1;">Specific Users</span> ({})', count)
    ...
    def activate_campaigns(self, request, queryset):
        ...
        self.message_user(request, f"{count} campaign(s) have been activated.", messages.SUCCESS)
    ...
    def deactivate_campaigns(self, request, queryset):
        ...
        self.message_user(request, f"{count} campaign(s) have been deactivated.", messages.SUCCESS)
    ...
    def send_test_emails(self, request, queryset):
        ...
        for campaign in queryset:
            try:
                ...
                self.message_user(
                    request, f"Failed to send test email for {campaign.name}: {e}", messages.ERROR
                )
...
class CampaignDeliveryAdmin(admin.ModelAdmin):
    ...
    list_filter = ["delivery_status", "campaign", "sent_at", "opened_at", "clicked_at"]
    search_fields = ["campaign__name", "user__email", "recipient_email", "email_subject"]
    ordering = ["-sent_at"]
...
class VideoViewAdmin(admin.ModelAdmin):
    ...
    list_filter = ["completed", "video__category", "started_at"]
    ...
    def completion_colored(self, obj):
        ...
        if obj.completed:
            ...
            return format_html(
                '<span style="color: #f5222d;">{}%</span>', obj.completion_percentage
            )
