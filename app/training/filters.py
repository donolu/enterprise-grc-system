...
class TrainingVideoFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = TrainingVideo
        fields = ["category", "video_provider", "difficulty_level", "is_published"]
    ...
    def filter_popular(self, queryset, name, value):
        ...
        if value:
            ...
            threshold = (
                queryset.aggregate(threshold=Percentile("view_count", 0.75))["threshold"] or 0
            )
...
class CampaignFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        fields = ["is_active", "send_frequency", "send_to_all_users", "created_by"]
    ...
    def filter_status(self, queryset, name, value):
        ...
        if value == "active":
            return queryset.filter(is_active=True, start_date__lte=now).filter(
            ...
            return queryset.filter(end_date__isnull=False, end_date__lte=now)
        elif value == "ready_to_send":
            return queryset.filter(is_active=True, next_send_date__lte=now).filter(
                Q(end_date__isnull=True) | Q(end_date__gt=now)
            )
    ...
    def filter_has_target_users(self, queryset, name, value):
        ...
        if value:
            return queryset.filter(send_to_all_users=False, target_users__isnull=False).distinct()
        else:
            return queryset.filter(Q(send_to_all_users=True) | Q(target_users__isnull=True))

    def filter_high_open_rate(self, queryset, name, value):
        """Filter campaigns with high open rates (>30%)."""
        if value:
            return queryset.filter(total_sent__gt=0).extra(
    ...
    def filter_due_to_send(self, queryset, name, value):
        ...
        if value:
            return queryset.filter(is_active=True, next_send_date__lte=timezone.now()).filter(
    ...
    def filter_overdue(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.filter(is_active=True, next_send_date__lt=overdue_threshold).filter(
                Q(end_date__isnull=True) | Q(end_date__gt=timezone.now())
            )
        return queryset
