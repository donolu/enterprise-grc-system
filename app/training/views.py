...
class TrainingVideoViewSet(viewsets.ModelViewSet):
    ...
    def track_view(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Video view tracked successfully", "view": serializer.data},
    ...
    def analytics(self, request):
        ...
        # Most watched videos
        most_watched = videos.annotate(views_count=Count("views")).order_by("-views_count")[:5]
        ...
        # Completion rate
        completion_rate = VideoView.objects.filter(video__in=videos, completed=True).count()
...
class SecurityAwarenessCampaignViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            SecurityAwarenessCampaign.objects.select_related("created_by")
            .prefetch_related("target_users", "deliveries")
            .all()
        )
    ...
    def send_now(self, request, pk=None):
        ...
        return Response({"message": "Campaign queued for sending", "task_id": task.id})
    ...
    def test_send(self, request, pk=None):
        ...
        return Response({"message": f"Test email sent to {request.user.email}", "task_id": task.id})
    ...
    def due_to_send(self, request):
        ...
        return Response({"count": due_campaigns.count(), "campaigns": serializer.data})
    ...
    def analytics(self, request):
        ...
        avg_open_rate = (
            campaigns_with_sends.aggregate(avg=Avg("total_opened") / Avg("total_sent") * 100)["avg"]
            or 0
        )
        avg_click_rate = (
            campaigns_with_sends.aggregate(avg=Avg("total_clicked") / Avg("total_sent") * 100)[
                "avg"
            ]
            or 0
        )
        ...
        # Recent activity
        recent_deliveries = CampaignDelivery.objects.select_related("campaign", "user").order_by(
            "-sent_at"
        )[:10]
...
def training_dashboard(request):
    ...
    total_campaigns = campaigns.count()
    active_campaigns = campaigns.filter(is_active=True).count()
    campaigns_due = campaigns.filter(is_active=True, next_send_date__lte=timezone.now()).count()

    # Email analytics
    total_emails_sent = campaigns.aggregate(total=models.Sum("total_sent"))["total"] or 0
    ...
    if campaigns_with_sends.exists():
        total_sent_all = campaigns_with_sends.aggregate(total=models.Sum("total_sent"))["total"]
        total_opened_all = (
            campaigns_with_sends.aggregate(total=models.Sum("total_opened"))["total"] or 0
        )
        total_clicked_all = (
            campaigns_with_sends.aggregate(total=models.Sum("total_clicked"))["total"] or 0
        )
