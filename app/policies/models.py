...
class PolicyCategory(models.Model):
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text="Category name (e.g., Security, HR, Legal, IT)"
    )
    description = models.TextField(blank=True, help_text="Description of this policy category")
    color = models.CharField(
        max_length=7, default="#6366f1", help_text="Hex color code for visual identification"
    )
...
class Policy(models.Model):
    ...
    policy_code = models.CharField(
        max_length=20, unique=True, help_text="Unique policy code (e.g., POL-SEC-001)"
    )
    title = models.CharField(max_length=200, help_text="Policy title")
    category = models.ForeignKey(PolicyCategory, on_delete=models.PROTECT, related_name="policies")
    policy_type = models.CharField(max_length=20, choices=POLICY_TYPE_CHOICES, default="policy")
    ...
    next_review_date = models.DateField(
        null=True, blank=True, help_text="When this policy is due for review"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.next_review_date and self.review_frequency_months:
            ...
            self.next_review_date = timezone.now().date() + relativedelta(
...
class PolicyVersion(models.Model):
    ...
    version_number = models.CharField(
        max_length=10, help_text="Version number (e.g., 1.0, 1.1, 2.0)"
    )
    ...
    document_size = models.PositiveIntegerField(
        null=True, blank=True, help_text="Document size in bytes"
    )
    ...
    # Version metadata
    summary = models.TextField(blank=True, help_text="Summary of changes in this version")
    ...
    is_published = models.BooleanField(
        default=False, help_text="Whether this version is published and available to users"
    )
    ...
    effective_date = models.DateField(
        default=timezone.now, help_text="When this version becomes effective"
    )
    expiry_date = models.DateField(
        null=True, blank=True, help_text="When this version expires (optional)"
    )

    # Metadata
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_policy_versions"
    )
...
class PolicyVersionAuditLog(models.Model):
    ...
    policy_version = models.ForeignKey(
        PolicyVersion, on_delete=models.CASCADE, related_name="audit_logs"
    )
...
class PolicyAcknowledgment(models.Model):
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address when acknowledgment was made"
    )
    user_agent = models.TextField(blank=True, help_text="Browser user agent string")

    # Validity
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="When this acknowledgment expires"
    )
...
class PolicyDistribution(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    policy_version = models.ForeignKey(
        PolicyVersion, on_delete=models.CASCADE, related_name="distributions"
    )

    # Distribution details
    distributed_to = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="received_policies"
    )
    distributed_at = models.DateTimeField(auto_now_add=True)
    distributed_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="distributed_policies"
    )
