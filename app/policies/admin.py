...
from .models import PolicyCategory, Policy, PolicyVersion, PolicyAcknowledgment, PolicyDistribution
...
class PolicyCategoryAdmin(admin.ModelAdmin):
    ...
    list_display = ["name", "colored_name", "policies_count", "description_preview", "created_at"]
    list_filter = ["created_at"]
    search_fields = ["name", "description"]
    readonly_fields = ["created_at", "updated_at"]

    fieldsets = (
        ("Category Information", {"fields": ("name", "description", "color")}),
...
class PolicyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            "Policy Information",
            {"fields": ("policy_code", "title", "category", "policy_type", "status")},
        ),
        ("Management", {"fields": ("owner", "approver")}),
        (
            "Review Schedule",
            {"fields": ("review_frequency_months", "next_review_date", "is_due_for_review")},
        ),
        (
            "Acknowledgment Settings",
            {"fields": ("requires_acknowledgment", "acknowledgment_validity_days")},
        ),
        (
            "Version Information",
            {"fields": ("current_version", "latest_version"), "classes": ("collapse",)},
        ),
        (
            "Metadata",
    ...
    actions = ["mark_as_approved", "mark_as_under_review", "mark_as_draft", "export_policies_csv"]
    ...
    def current_version_display(self, obj):
        ...
        if current:
            return format_html('<span title="Active version">v{}</span>', current.version_number)
...
class PolicyVersionAdmin(admin.ModelAdmin):
    ...
    list_filter = ["is_active", "is_published", "policy__category", "created_at", "effective_date"]
    search_fields = ["policy__title", "policy__policy_code", "version_number", "summary"]
    readonly_fields = [
        "document_size",
        "file_name",
        "file_extension",
    ...
    fieldsets = (
        ("Version Information", {"fields": ("policy", "version_number", "summary")}),
        ("Document", {"fields": ("document", "document_size", "file_name", "file_extension")}),
        ("Status", {"fields": ("is_active", "is_published", "is_current", "is_expired")}),
        ("Dates", {"fields": ("effective_date", "expiry_date")}),
        ("Approval", {"fields": ("approved_at", "approved_by")}),
        ("Metadata", {"fields": ("created_at", "created_by"), "classes": ("collapse",)}),
    )
    ...
    def acknowledgment_count(self, obj):
        ...
        if count > 0:
            url = (
                reverse("admin:policies_policyacknowledgment_changelist")
                + f"?policy_version__id__exact={obj.id}"
            )
    ...
    def activate_versions(self, request, queryset):
        ...
        for version in queryset:
            ...
            PolicyVersion.objects.filter(policy=version.policy, is_active=True).update(
                is_active=False
            )
...
class PolicyAcknowledgmentAdmin(admin.ModelAdmin):
    ...
    search_fields = [
        ...
        "user__first_name",
        "user__last_name",
        "policy_version__policy__title",
        "policy_version__policy__policy_code",
    ]
    readonly_fields = ["acknowledged_at", "is_expired", "is_valid"]
    date_hierarchy = "acknowledged_at"

    fieldsets = (
        ("Acknowledgment Information", {"fields": ("user", "policy_version", "acknowledged_at")}),
        ("Validity", {"fields": ("expires_at", "is_expired", "is_valid")}),
        ("Metadata", {"fields": ("ip_address", "user_agent"), "classes": ("collapse",)}),
    )
    ...
    def policy_info(self, obj):
        ...
        return format_html(
            '<a href="{}">{}</a><br><small style="color: #6b7280;">v{}</small>',
            policy_url,
    ...
    def validity_status(self, obj):
        ...
        if obj.is_expired:
            ...
            if days_left <= 30:
                ...
                return format_html(
                    '<span style="color: #10b981;">✅ Valid ({} days left)</span>', days_left
                )
...
class PolicyDistributionAdmin(admin.ModelAdmin):
    ...
    search_fields = [
        "distributed_to__email",
        "distributed_to__first_name",
        "distributed_to__last_name",
        "policy_version__policy__title",
        "policy_version__policy__policy_code",
    ]
    readonly_fields = ["distributed_at", "notification_sent_at", "acknowledged_at", "is_overdue"]
    date_hierarchy = "distributed_at"

    fieldsets = (
        (
            "Distribution Information",
            {"fields": ("policy_version", "distributed_to", "distributed_by", "distributed_at")},
        ),
        ("Status", {"fields": ("acknowledged", "acknowledged_at", "is_overdue")}),
        (
            "Notifications",
            {
                "fields": (
    ...
    def status_display(self, obj):
        ...
        if obj.acknowledged:
            ...
            return format_html('<span style="color: #ef4444; font-weight: 500;">🚨 Overdue</span>')
        else:
            return format_html('<span style="color: #f59e0b; font-weight: 500;">⏳ Pending</span>')
    ...
    def reminder_info(self, obj):
        ...
        if obj.reminder_count > 0:
            return format_html(
                '{} reminders<br><small style="color: #6b7280;">Last: {}</small>',
                obj.reminder_count,
                obj.last_reminder_sent.strftime("%Y-%m-%d")
