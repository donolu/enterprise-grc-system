...
class PolicyFilter(django_filters.FilterSet):
    ...
    # Status filters
    status = django_filters.MultipleChoiceFilter(choices=Policy.APPROVAL_STATUS_CHOICES)
    policy_type = django_filters.MultipleChoiceFilter(choices=Policy.POLICY_TYPE_CHOICES)
    ...
    def filter_due_for_review(self, queryset, name, value):
        ...
        if value:
            return queryset.filter(next_review_date__lte=timezone.now().date())
        return queryset
    ...
    def filter_review_overdue(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.filter(next_review_date__lte=overdue_date)
        return queryset
...
class PolicyVersionFilter(django_filters.FilterSet):
    ...
    policy_code = django_filters.CharFilter(
        field_name="policy__policy_code", lookup_expr="icontains"
    )
    policy_title = django_filters.CharFilter(field_name="policy__title", lookup_expr="icontains")
    ...
    class Meta:
        model = PolicyVersion
        fields = ["policy", "is_active", "is_published", "version_number"]
...
class PolicyAcknowledgmentFilter(django_filters.FilterSet):
    ...
    policy_code = django_filters.CharFilter(
        field_name="policy_version__policy__policy_code", lookup_expr="icontains"
    )
    ...
    def filter_expires_soon(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.filter(expires_at__lte=expire_date, expires_at__gte=timezone.now())
        return queryset
...
class PolicyDistributionFilter(django_filters.FilterSet):
    ...
    user_email = django_filters.CharFilter(
        field_name="distributed_to__email", lookup_expr="icontains"
    )
    ...
    distributed_after = django_filters.DateTimeFilter(
        field_name="distributed_at", lookup_expr="gte"
    )
    distributed_before = django_filters.DateTimeFilter(
        field_name="distributed_at", lookup_expr="lte"
    )

    # Reminder filters
    needs_reminder = django_filters.BooleanFilter(method="filter_needs_reminder")
    reminder_count_gte = django_filters.NumberFilter(field_name="reminder_count", lookup_expr="gte")
    ...
    def filter_is_overdue(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.filter(distributed_at__lte=overdue_date, acknowledged=False)
        return queryset
    ...
    def filter_needs_reminder(self, queryset, name, value):
        ...
        if value:
            ...
            return queryset.filter(
                Q(last_reminder_sent__isnull=True) | Q(last_reminder_sent__lte=reminder_date),
                acknowledged=False,
            )
        return queryset
