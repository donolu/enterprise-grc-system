...
class PolicyCategoryViewSet(viewsets.ModelViewSet):
    ...
    def policies(self, request, pk=None):
        ...
        policies = (
            Policy.objects.filter(category=category)
            .select_related("category", "owner", "approver")
...
class PolicyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            Policy.objects.select_related("category", "owner", "approver", "created_by")
            .prefetch_related("versions__created_by", "versions__approved_by")
            .all()
        )
    ...
    def summary(self, request):
        ...
        policies_due_review = policies.filter(next_review_date__lte=timezone.now().date()).count()
        ...
        # Recent versions
        recent_versions = PolicyVersion.objects.filter(created_at__gte=recent_date).select_related(
            "policy"
        )[:5]
    ...
    def versions(self, request, pk=None):
        ...
        versions = policy.versions.select_related("created_by", "approved_by").all()
    ...
    def acknowledge(self, request, pk=None):
        ...
        existing = PolicyAcknowledgment.objects.filter(
            user=request.user, policy_version=current_version
        ).first()

        if existing and existing.is_valid:
            return Response(
                {"error": "Policy already acknowledged"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Create acknowledgment
        serializer = PolicyAcknowledgmentCreateSerializer(
            data={"policy_version": current_version.id}, context={"request": request}
        )
        ...
        if serializer.is_valid():
            ...
            distribution = PolicyDistribution.objects.filter(
                policy_version=current_version, distributed_to=request.user
            ).first()
            ...
            return Response(
                PolicyAcknowledgmentSerializer(acknowledgment).data, status=status.HTTP_201_CREATED
            )
    ...
    def distribute(self, request, pk=None):
        ...
        for user in users:
            distribution, created = PolicyDistribution.objects.get_or_create(
                ...
                distributed_to=user,
                defaults={"distributed_by": request.user, "notification_sent": False},
            )
    ...
    def acknowledgment_dashboard(self, request):
        ...
        policies = (
            ...
            .prefetch_related("versions__acknowledgments", "versions__distributions")
        )
        ...
        for policy in policies:
            ...
            acknowledgment_rate = (
                (total_acknowledged / total_distributed * 100) if total_distributed > 0 else 0.0
            )

            # Get pending acknowledgments
            pending_users = distributions.filter(acknowledged=False).select_related(
                "distributed_to"
            )
    ...
    def acknowledgment_status(self, request, pk=None):
        ...
        if not current_version:
            return Response(
                {"error": "No active version available"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Get all distributions for current version
        distributions = (
            PolicyDistribution.objects.filter(policy_version=current_version)
            .select_related("distributed_to")
            .order_by("-distributed_at")
        )

        # Get all acknowledgments for current version
        acknowledgments = (
            PolicyAcknowledgment.objects.filter(policy_version=current_version)
            .select_related("user")
    ...
    def my_policies(self, request):
        ...
        distributions = (
            PolicyDistribution.objects.filter(distributed_to=request.user, acknowledged=False)
            .select_related("policy_version__policy__category")
            .order_by("-distributed_at")
        )
        ...
        return Response({"count": len(policies_data), "policies": policies_data})
...
class PolicyVersionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            PolicyVersion.objects.select_related("policy__category", "created_by", "approved_by")
    ...
    def activate(self, request, pk=None):
        ...
        PolicyVersion.objects.filter(policy=version.policy, is_active=True).update(is_active=False)
    ...
    def download(self, request, pk=None):
        ...
        if version.lifecycle_state == "final":
            if not version.final_pdf:
                return Response(
                    {"error": "Final PDF is not available"}, status=status.HTTP_409_CONFLICT
                )
            log_policy_version_event(
                version=version,
...
def _source_file_response(version, request):
    if not version.document:
        return Response({"error": "No document available"}, status=status.HTTP_404_NOT_FOUND)
    ...
    return FileResponse(
        ...
        or f"{version.policy.policy_code}_v{version.version_number}{version.file_extension or ''}",
    )
...
class PolicyDistributionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return PolicyDistribution.objects.select_related(
            "policy_version__policy__category", "distributed_to", "distributed_by"
        ).all()
    ...
    def pending(self, request):
        ...
        distributions = self.get_queryset().filter(distributed_to=request.user, acknowledged=False)
