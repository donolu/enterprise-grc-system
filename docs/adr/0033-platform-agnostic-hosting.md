...
- **Storage backend by configuration.** Select the tenant-aware storage backend from the environment (Azure Blob, S3, DigitalOcean Spaces, Cloudflare R2, MinIO/local filesystem), rather than hardcoding Azure Blob. Tenant isolation of stored objects is preserved across backends.
